...
pub(crate) use upload::{
    redact_remote_prepared_request, upload_prepared_sessions_to_server_with_auth,
};
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) struct SessionWatchProcessEvent {
...
pub(crate) trait SessionWatchEventSink: Send + Sync {
    fn emit(&self, event: SessionWatchProcessEvent);
}

pub(crate) struct NoopSessionWatchEventSink;
...
#[async_trait]
pub(crate) trait SessionWatchIngestor: Send + Sync {
...
type ParseResult = (
    ...
    SessionFileMetadata,
    Option<String>,
    Result<Option<crate::ingest::sessions::SessionDoc>, String>,
);
...
async fn prepare_session_docs_for_watch(
    ...
    for (index, validated, meta, known_hash) in candidates {
        ...
        tasks.push(async move {
            ...
            let parsed = crate::ingest::sessions::collect_session_file_doc(&cfg, &validated)
                .await
...
pub(crate) enum WatchIngestResult {
    Completed(String),
