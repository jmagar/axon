...
fn private_ip_blocked() {
    assert!(
        crate::core::http::validate_url("http://192.168.0.1/api/v1/repos/o/r").is_err(),
        "RFC-1918 192.168.x must be blocked"
    );
    assert!(
        crate::core::http::validate_url("http://172.16.0.1/api/v1").is_err(),
        "RFC-1918 172.16.x must be blocked"
    );
    assert!(
        crate::core::http::validate_url("http://169.254.169.254/latest/meta-data/").is_err(),
...
fn public_codeberg_url_allowed() {
    assert!(
        crate::core::http::validate_url("https://codeberg.org/api/v1/repos/owner/repo").is_ok(),
