...
use crate::mcp::schema::{MemoryNodeType, MemoryRequest, MemorySubaction};
...
fn parse_edge_type(value: &str) -> Result<crate::mcp::schema::MemoryEdgeType, Box<dyn Error>> {
    match value {
        "relates_to" => Ok(crate::mcp::schema::MemoryEdgeType::RelatesTo),
        "supersedes" => Ok(crate::mcp::schema::MemoryEdgeType::Supersedes),
...
mod tests {
    ...
    fn parses_link_request() {
        ...
        assert!(matches!(
            req.edge_type,
            Some(crate::mcp::schema::MemoryEdgeType::Supersedes)
        ));
    }
