...
#[utoipa::path(
    ...
    responses(
        ...
        (status = 500, description = "Session store error", body = crate::web::server::error::ErrorBody)
    ),
    tag = "mobile"
)]
pub async fn list_mobile_sessions(
    ...
    crate::services::mobile_sessions::list_sessions(&owner)
        .await
        .map(Json)
...
#[utoipa::path(
    ...
    responses(
        ...
        (status = 404, description = "Session not found", body = crate::web::server::error::ErrorBody)
    ),
    tag = "mobile"
)]
pub async fn get_mobile_session(
    ...
    let owner = mobile_session_owner(auth.as_ref().map(|Extension(auth)| auth));
    crate::services::mobile_sessions::get_session(&owner, &id)
        .await
        .map(Json)
        .map_err(map_mobile_session_error)
}
...
#[utoipa::path(
    ...
    responses(
        ...
        (status = 400, description = "Invalid session payload", body = crate::web::server::error::ErrorBody),
        (status = 409, description = "Stale mobile session update", body = crate::web::server::error::ErrorBody)
    ),
    tag = "mobile"
)]
pub async fn upsert_mobile_session(
    ...
    let owner = mobile_session_owner(auth.as_ref().map(|Extension(auth)| auth));
    crate::services::mobile_sessions::upsert_session(&owner, &id, request)
        .await
        .map(Json)
        .map_err(map_mobile_session_error)
}
...
pub async fn delete_mobile_session(
    ...
    let owner = mobile_session_owner(auth.as_ref().map(|Extension(auth)| auth));
    crate::services::mobile_sessions::delete_session(&owner, &id)
        .await
        .map(Json)
        .map_err(map_mobile_session_error)
}
