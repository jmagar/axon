...
const EDIT_TABLE_MULTI_CELL_EXAMPLES = [
  // 1) Simple text edit
  dedent`
    <instruction>
    Fix grammar
...
const EDIT_TABLE_MULTI_CELL_RULES = dedent`
...
const EDIT_TABLE_MULTI_CELL_TASK = dedent`
...
export function buildEditTableMultiCellPrompt(
  ...
  return buildStructuredPrompt({
    ...
    examples: EDIT_TABLE_MULTI_CELL_EXAMPLES,
    history: formatTextFromMessages(messages),
    instruction: getLastUserInstruction(messages),
    rules: EDIT_TABLE_MULTI_CELL_RULES,
    task: EDIT_TABLE_MULTI_CELL_TASK,
  })
}
