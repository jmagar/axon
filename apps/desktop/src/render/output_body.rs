...
pub(crate) fn render_output_body(output: CommandOutput, errors_open: bool) -> impl IntoElement {
    ...
    div()
        ...
        .when(has_stdout && has_stderr, |el| {
            el.child(render_output_tabs(errors_open))
        })
        .when(empty, |el| {
            el.child(
                div()
...
fn render_output_tabs(errors_open: bool) -> impl IntoElement {
    div()
        .flex()
        .flex_row()
        .items_center()
        ...
        .rounded_sm()
        .bg(rgb(AURORA_CONTROL_SURFACE))
        .child(render_output_tab("stdout", !errors_open).on_mouse_down(
            MouseButton::Left,
        ...
        .child(render_output_tab("errors", errors_open).on_mouse_down(
            MouseButton::Left,
...
fn render_output_tab(label: &'static str, active: bool) -> Div {
    div()
        .px_2()
        .py_1()
        .rounded_sm()
