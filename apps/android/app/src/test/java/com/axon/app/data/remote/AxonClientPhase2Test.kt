...
class AxonClientPhase2Test {
    ...
    @Test fun `ingestStart posts to v1 sources and decodes AcceptedJob`() = runBlocking {
        server.enqueue(MockResponse().setBody("""{"job_id":"abc","canonical_uri":"https://github.com/o/r","status":"queued"}""").addHeader("Content-Type","application/json"))
        ...
        assertTrue(body.contains("\"adapter\":\"github\""))
    }
    ...
    @Test fun `getJob reads unified job summary`() = runBlocking {
        server.enqueue(MockResponse().setBody("""{"job_id":"j","kind":"source","status":"completed","phase":"complete","created_at":"2026-01-01T00:00:00Z","updated_at":"2026-01-01T00:00:00Z"}""").addHeader("Content-Type","application/json"))
    ...
    @Test fun `ingestList GETs v1 jobs list and decodes ServiceJob array`() = runBlocking {
        server.enqueue(MockResponse().setBody("""{"items":[{"job_id":"j","kind":"source","status":"completed","phase":"complete","created_at":"2026-01-01T00:00:00Z","updated_at":"2026-01-01T00:00:00Z"}],"limit":100}""").addHeader("Content-Type","application/json"))
    ...
    @Test fun `listJobs decodes progress json`() = runBlocking {
        server.enqueue(
            MockResponse()
                .setBody(
                    """{"items":[{"job_id":"j","kind":"source","status":"running","phase":"fetching","created_at":"2026-01-01T00:00:00Z","updated_at":"2026-01-01T00:00:00Z"}],"limit":25}"""
                )
        ...
        assertTrue(r.isSuccess)
        assertEquals("running", r.getOrThrow()[0].status)
    }
    ...
    @Test fun `cancelJob POSTs v1 jobs id cancel`() = runBlocking {
        server.enqueue(MockResponse().setBody("""{"job_id":"j1","status":"canceled"}""").addHeader("Content-Type","application/json"))
