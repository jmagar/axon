...
class AxonClientErrorPathTest {
    ...
    @Test fun `summarize surfaces 401 unauthorized`() =
        runBlocking {
    ...
    @Test fun `summarize surfaces 403 forbidden`() =
        runBlocking {
    ...
    @Test fun `searchWeb surfaces 500 internal server error with truncated body`() =
        runBlocking {
    ...
    @Test fun `cancelJob surfaces 404 not found`() =
        runBlocking {
            server.enqueue(MockResponse().setResponseCode(404))
            val r = client.cancelJob(AxonClient.JobKind.Source, "missing")
            assertTrue(r.isFailure)
            assertTrue(r.exceptionOrNull()?.message?.contains("404") == true)
        }

    @Test fun `status surfaces 503 service unavailable`() =
        runBlocking {
    ...
    @Test fun `sourceSubmit surfaces malformed JSON as failure`() =
        runBlocking {
            server.enqueue(
                MockResponse()
                    .setResponseCode(202)
    ...
    @Test fun `summarize surfaces wrong-shape JSON as failure`() =
        runBlocking {
    ...
    @Test fun `searchWeb surfaces empty body as failure`() =
        runBlocking {
    ...
    @Test fun `listJobs surfaces abrupt disconnect as failure`() =
        runBlocking {
    ...
    @Test fun `doctor surfaces abrupt disconnect as failure`() =
        runBlocking {
            server.enqueue(MockResponse().apply { socketPolicy = SocketPolicy.DISCONNECT_AT_START })
