...
class AxonRepositoryPhase2Test {
    ...
    @Test fun `summarize maps wire to UI`() =
        runBlocking {
            server.enqueue(
                MockResponse()
                    .setBody(
                        """{"urls":["a"],"summary":"hi","context_chars":7,"context_truncated":false}""",
    ...
    @Test fun `searchWeb maps results and source jobs`() =
        runBlocking {
            server.enqueue(
                MockResponse()
                    .setBody(
                        """{"query":"k","results":[{"title":"t","url":"https://x"}],"source_jobs":[{"job_id":"j","url":"https://x"}]}""",
                    ).addHeader("Content-Type", "application/json"),
            )
            val r = repo.searchWeb("k").getOrThrow()
            assertEquals(1, r.results.size)
            assertEquals("j", r.sourceJobs[0].jobId)
        }

    @Test fun `source submission returns jobId`() =
        runBlocking {
            server.enqueue(
                MockResponse()
                    .setResponseCode(
            ...
            assertEquals("abc", repo.sourceSubmit("https://github.com/o/r").getOrThrow())
        }

    @Test fun `extractStart returns jobId`() =
        runBlocking {
            server.enqueue(
                MockResponse()
                    .setResponseCode(
                        202,
                    ).setBody("""{"job_id":"ex","status":"pending"}""")
                    .addHeader("Content-Type", "application/json"),
            )
    ...
    @Test fun `listJobs returns full server array unchanged (no client-side slicing)`() =
        runBlocking {
            server.enqueue(
                MockResponse()
                    .setBody(
                        """{"items":[{"job_id":"a","kind":"source","status":"x"},{"job_id":"b","kind":"source","status":"y"}],"limit":25}""",
                    ).addHeader("Content-Type", "application/json"),
            )
            val jobs = repo.listJobs(JobFamily.Source).getOrThrow()
            assertEquals(2, jobs.size)
        }

    @Test fun `listJobs maps progressJson to JobUi`() =
        runBlocking {
            server.enqueue(
                MockResponse()
                    .setBody(
            ...
            val job = repo.listJobs(JobFamily.Source).getOrThrow().single()
    ...
    @Test fun `listWatches maps watch definitions`() =
        runBlocking {
            server.enqueue(
                MockResponse()
                    .setBody(
                        """{"items":[{"watch_id":"w","source_id":"Docs","enabled":true,"schedule":{"every_seconds":300}}]}""",
                    ).addHeader("Content-Type", "application/json"),
            )
    ...
    @Test fun `summarize blocked by missing token`() =
        runBlocking {
            val r2 =
                AxonRepository(
