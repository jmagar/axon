...
internal fun AskViewModel.askFromQuery(
    query: String,
...
internal fun userFacingAskError(message: String): String {
    ...
    return when {
        detail.contains("No candidates passed topical overlap", ignoreCase = true) -> {
            "Error: I couldn't find indexed context for this question. Switch to Chat for a general answer, or use + to Search, Index site, Source, or Retrieve the relevant material first.\n\nDetail: $detail"
        }
        ...
        detail.contains("No response received from server", ignoreCase = true) -> {
            ...
            "Error: Axon couldn't complete this request. Retry, switch modes, or use + to gather the source material first.\n\nDetail: $detail"
        }
    }
}
