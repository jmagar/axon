 ...
 * `Authorization: Bearer …` line in a site-source form would land on disk in the clear.
 *
 * This store uses the same AES-256-GCM scheme as [EncryptedTokenStore]. Header
 * lists are persisted per *mode key* (site-source headers, Ask headers, etc.) as a
...
class EncryptedHeadersStore(
    ...
    fun read(key: String): List<String>? {
        ...
        val raw =
            runCatching { p.getString(key, null) }
                .getOrElse { t ->
    ...
    companion object {
        ...
        const val KEY_SITE_SOURCE_HEADERS = "source_site.headers"
    }
}
