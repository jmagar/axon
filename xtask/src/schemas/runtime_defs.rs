...
pub fn database_artifacts(root: &Path) -> Result<Vec<SchemaArtifact>> {
    ...
    let inputs = source_inputs(root, spec.source_paths)?;
    let (parsed_fields, summary) = database_defs::build_artifact_fields(root)?;
    let mut schema = schema_bundle(
        schema_id(SchemaFamily::Database),
        spec.title,
        "cargo xtask schemas database",
        spec.owner_crates,
        &inputs,
        legacy_unified_jobs_observability_defs(),
    );
    {
        let object = schema
            .as_object_mut()
            .expect("database schema bundle is an object");
        object.insert("schema_version".to_string(), json!(1));
        for (key, value) in parsed_fields
            .as_object()
            .expect("parsed database fields are an object")
        {
    ...
    Ok(vec![
        ...
        SchemaArtifact::new(
            ...
            database_markdown(&inputs, &summary),
        ),
    ])
}

/// Per-table `x-axon` detail for the legacy unified-jobs observability def.
/// Split out of `legacy_unified_jobs_observability_defs` to stay under the
/// repo's 120-line function cap.
fn legacy_unified_jobs_observability_tables() -> Value {
    json!({
        "jobs": {
            "primary_key": ["job_id"],
...
/// Retained for the pre-existing "Unified Job Tables" markdown section and
/// as a compact human-curated cross-check def alongside the fully parsed
/// `tables`/`indexes`/`foreign_keys`/`migrations` root arrays above.
fn legacy_unified_jobs_observability_defs() -> Value {
    json!({
        "UnifiedJobsObservabilitySchema": {
            ...
            "x-axon": {
                ...
                "tables": legacy_unified_jobs_observability_tables()
            }
        }
    })
}
