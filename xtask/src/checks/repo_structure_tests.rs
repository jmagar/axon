...
fn broken_agent_memory_symlink_fails() {
    let fixture = complete_fixture();
    fs::remove_file(fixture.root.join("crates/axon-document/src/AGENTS.md")).unwrap();
    write_symlink(
        "../CLAUDE.md",
        &fixture.root.join("crates/axon-document/src/AGENTS.md"),
    );
...
fn missing_target_module_fails() {
    let fixture = complete_fixture();
    fs::remove_file(fixture.root.join("crates/axon-document/src/preparer.rs")).unwrap();

    let err = check_root(&fixture.root).unwrap_err();
    assert!(err.contains("preparer.rs"), "{err}");
}
...
fn missing_target_module_declaration_fails() {
    let fixture = complete_fixture();
    write(
        &fixture.root.join("crates/axon-document/src/lib.rs"),
        "pub const CRATE_NAME: &str = \"axon-document\";\n",
    );

    let err = check_root(&fixture.root).unwrap_err();
    assert!(
        err.contains("lib.rs is missing module declaration: pub mod preparer;"),
        "{err}"
    );
}
...
fn unexpected_target_module_file_fails() {
    let fixture = complete_fixture();
    write(
        &fixture.root.join("crates/axon-document/src/surprise.rs"),
...
fn target_dependency_fails() {
    let fixture = complete_fixture();
    write(
        &fixture.root.join("crates/axon-document/Cargo.toml"),
        "[package]\nname = \"axon-document\"\nrust-version.workspace = true\n\n[dependencies]\naxon-services = { path = \"../axon-services\" }\n",
    );

    let err = check_root(&fixture.root).unwrap_err();
    assert!(
        err.contains("PR0 target crate axon-document must keep [dependencies] empty"),
        "{err}"
    );
}
...
fn target_specific_dependency_fails() {
    ...
    assert!(
        err.contains(
            "PR0 target crate axon-document must keep [target.cfg(unix).dependencies] empty"
        ),
        "{err}"
    );
}
...
fn target_package_name_fails() {
    ...
    let err = check_root(&fixture.root).unwrap_err();
    assert!(
        err.contains("PR0 target crate axon-document must set package.name = \"axon-document\""),
        "{err}"
    );
}
...
fn missing_target_rust_version_fails() {
    ...
    let err = check_root(&fixture.root).unwrap_err();
    assert!(
        err.contains("PR0 target crate axon-document must set rust-version.workspace = true"),
        "{err}"
    );
}
