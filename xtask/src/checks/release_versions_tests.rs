...
#[test]
fn cargo_lock_package_version_reader_and_replacer_target_one_package() {
...
"#;
    ...
    let updated = replace_cargo_lock_package_version(content, Some("axon"), "1.2.4")
        .expect("replace lock version");
    assert!(updated.contains("name = \"axon\"\nversion = \"1.2.4\""));
...
#[test]
fn npm_package_lock_version_reader_and_replacer_updates_root_package() {
    let content = r#"{
  ...
  "packages": {
    ...
    let updated = replace_npm_package_lock_version(content, Some("@axon/admin-panel"), "1.2.4")
        .expect("replace package-lock version");
    assert_eq!(
        read_npm_package_lock_version(&updated, Some("@axon/admin-panel")).expect("updated"),
        "1.2.4"
    );
}
...
fn bump_cli_updates_source_manifests_and_lockfiles() {
    ...
    assert_eq!(
        read_cargo_lock_package_version(
            &fs::read_to_string(fixture.path("Cargo.lock")).unwrap(),
            Some("axon")
        )
        .unwrap(),
        "1.0.1"
    ...
    assert!(
        fs::read_to_string(fixture.path("README.md"))
            .unwrap()
            .contains("Version: 1.0.1")
    );
    assert!(
        fs::read_to_string(fixture.path("CHANGELOG.md"))
            .unwrap()
            .contains("## [1.0.1] - ")
    );
    assert_eq!(
    ...
    assert_eq!(
        read_npm_package_lock_version(
            &fs::read_to_string(fixture.path("apps/web/package-lock.json")).unwrap(),
            Some("@axon/admin-panel")
        )
        .unwrap(),
        "1.0.1"
    );
    assert_eq!(
        fs::read_to_string(fixture.path("plugins/axon/.claude-plugin/plugin.json")).unwrap(),
        r#"{"name":"axon"}"#
    );
}
...
fn bump_palette_updates_source_manifests_and_lockfile() {
    ...
    assert_eq!(
        read_json_version(
            &fs::read_to_string(fixture.path("apps/palette-tauri/src-tauri/tauri.conf.json"))
                .unwrap(),
            Some("/version")
        )
        .unwrap(),
    ...
    assert_eq!(
        read_json_version(
            &fs::read_to_string(fixture.path("apps/palette-tauri/package.json")).unwrap(),
            Some("/version")
        )
        .unwrap(),
        "5.11.0"
    ...
    assert_eq!(
        read_cargo_lock_package_version(
            &fs::read_to_string(fixture.path("apps/palette-tauri/src-tauri/Cargo.lock")).unwrap(),
            Some("axon-palette-tauri")
        )
        .unwrap(),
        "5.11.0"
    );
