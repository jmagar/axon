...
// ── services::refresh — pool-based schedule sweep wrappers ───────────────────

#[test]
fn refresh_service_exposes_schedule_sweep_wrappers() {
    // Compile-time proof: the pool-based schedule sweep wrappers exist and are pub.
    let _ = axon::crates::services::refresh::refresh_ensure_schema;
    let _ = axon::crates::services::refresh::refresh_claim_due_schedules;
    let _ = axon::crates::services::refresh::refresh_mark_schedule_ran;
}

#[test]
fn refresh_service_exposes_pure_github_reingest_predicate() {
    // Pure function: fail-open when pushed_at cannot be parsed.
    let result =
        axon::crates::services::refresh::refresh_should_reingest_github("not-a-date", None);
    assert!(
        result,
        "should_reingest_github must return true when pushed_at is unparseable (fail-open)"
    );
}

#[test]
fn refresh_service_reingest_github_first_run_always_true() {
    // First run (last_run_at = None) should always trigger a re-ingest.
    let result = axon::crates::services::refresh::refresh_should_reingest_github(
        "2026-01-01T00:00:00Z",
        None,
    );
    assert!(result, "first run should always trigger re-ingest");
}

#[test]
fn refresh_service_reingest_github_no_new_push_returns_false() {
    use chrono::{TimeZone, Utc};
    // last_run_at is AFTER pushed_at — no new push, should skip.
    let pushed_at = "2026-01-01T00:00:00Z";
    let last_run_at = Some(Utc.with_ymd_and_hms(2026, 2, 1, 0, 0, 0).unwrap());
    let result =
        axon::crates::services::refresh::refresh_should_reingest_github(pushed_at, last_run_at);
    assert!(!result, "no new push should not trigger re-ingest");
}
