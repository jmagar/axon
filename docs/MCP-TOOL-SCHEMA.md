...
- Lifecycle families: `artifacts`, `crawl`, `embed`, `extract`, `graph`, `ingest`, `refresh`
- Direct actions: `ask`, `doctor`, `domains`, `elicit_demo`, `export`, `help`, `map`, `query`, `research`, `retrieve`, `scrape`, `screenshot`, `search`, `sources`, `stats`, `status`
...
- `graph`: `build|status|explore|stats` -- build requires one of `url`, `domain`, or `all=true`
