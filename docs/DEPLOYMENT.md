...
- `AXON_MCP_HTTP_TOKEN` — static bearer for `/mcp` and `/v1/actions`
- `AXON_MCP_AUTH_MODE=oauth` plus Google OAuth vars — optional OAuth/JWT mode for MCP and first-party action routes
...
- Runtime routes live in `src/web/*` and include `/api/panel/*`, `/v1/ask`,
  `/v1/capabilities`, and `/v1/actions`.
