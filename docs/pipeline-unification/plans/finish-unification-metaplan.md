...
| Move vertical extraction parse facts into `axon-parse`/`axon-adapters`; keep structured `extract` orchestration in `axon-services`. | Partial / selectively deferred | Do not restore the deleted `axon-extract` vertical crate wholesale: the unification target is source routing + parser/enrichment output ownership. `DocumentPreparer` calls `axon_parse::builtins::production_registry()` when facts are absent, and parse facts flow into `PreparedDocument`. Registry package targets are partially re-homed through `pkg:npm`, `pkg:pypi`, and `pkg:crates` source adapters. Remaining old verticals (`github_pr`, `github_issue`, `github_repo`, `github_release`, `reddit`, `docs_rs`, `docker_hub`, `huggingface_model`, `dev_to`, `shopify`, `hackernews`, `stackoverflow`, `arxiv`, plus explicit-only `amazon`/`ebay`) are deferred until each is reintroduced as adapter/parse/enrichment output, not as source lifecycle or vector persistence owner. |
...
| Issue item | Verdict | Evidence |
|---|---:|---|
| Implement one durable job table family. | Open | `JobKind` still maps to `axon_crawl_jobs`, `axon_embed_jobs`, `axon_extract_jobs`, and `axon_ingest_jobs`; `enqueue_job` inserts into those tables; migration `0001_create_tables.sql` creates them. |
...
| Remove old `embed`, `ingest`, `scrape`, `crawl`, `code-search`, `code-search-watch`, `purge`, `dedupe`, and legacy MCP action families from normal public surfaces. | Partial/Open | `embed/ingest/scrape/crawl/code-search` are absent from current CLI parsing, but `purge`/`dedupe` remain in `CommandKind`, clap parsing, and CLI dispatch. Generated docs/clients still reference old REST/MCP routes/actions. |
| Update CLI help, `axon --help`, `axon help`, completions, and parse fixtures from generated CLI registry. | Partial/Open | CLI registry exists, but public help still includes `dedupe`/`purge`; docs still contain removed commands/routes. |
| Ensure MCP exposes one `axon` tool, strict action/subaction schema, no removed actions, shared envelopes, and action auth metadata. | Partial/Open | MCP has a single tool description, but stale docs still reference legacy actions and task mapping still mentions `CodeSearch`. Needs schema artifact audit. |
| Ensure REST OpenAPI includes every end-state route and excludes every removed route. | Open | Generated docs/clients still reference `/v1/scrape`, `/v1/crawl`, `/v1/embed`, `/v1/ingest`, `/v1/dedupe`, `/v1/purge`, and `/v1/watch/{id}/run`. |
...
| Remove old split crates from workspace: `axon-vector`, `axon-code-index`, `axon-crawl`, `axon-ingest`, `axon-extract`. | Open | All five crates are still workspace packages. |
...
| `axon-vector` split into `axon-document`, `axon-embedding`, `axon-vectors`, and `axon-retrieval`. | Partial/Open | Target crates exist, but `axon-vector` is still a workspace crate. |
| `axon-code-index` split into ledger/parse/document/jobs/vectors. | Partial/Open | Target pieces exist, but `axon-code-index` is still a workspace crate and still owns generation/cleanup debt logic. |
| `axon-crawl` split into adapters/route/ledger/document/jobs. | Partial/Open | Target pieces exist, but `axon-crawl` is still a workspace crate. |
| `axon-ingest` split into adapters/route/ledger/document/jobs. | Partial/Open | Target pieces exist, but `axon-ingest` is still a workspace crate. |
| `axon-extract` split into llm/parse/adapters/services. | Partial/Open | Runtime LLM providers moved, but `axon-extract` remains a workspace crate. |
| Move/wrap/rewrite/delete rules applied with no legacy facades. | Open | Old split crates remain, and compatibility/public surface removal is incomplete. |
...
| Root `Cargo.toml` contains only target crate set after cutover. | Open | Workspace still includes `axon-vector`, `axon-code-index`, `axon-crawl`, `axon-ingest`, and `axon-extract`. |
| Removed crates absent from workspace members after cutover. | Open | `cargo metadata` lists all five removed crates as active packages. |
...
- [ ] Ensure removed CLI commands are absent and cannot dispatch: `embed`, `ingest`, `scrape`, `crawl`, `code-search`, `code-search-watch`, `purge`, `dedupe`, `refresh`, and `fresh`.
...
- [ ] Remove `axon-vector`, `axon-code-index`, `axon-crawl`, `axon-ingest`, and `axon-extract` from the workspace only after replacement paths and tests pass.
