...
| `axon-memory` integration with shared preparation, payload, graph, and retrieval rules where applicable; memory is not a source adapter. | Implemented with batching follow-up | Memory remains a distinct vector namespace/retrieval path with SQLite metadata and graph mirrors; it is not routed as a source adapter. |
...
- [x] Keep memory as its own job kind, Qdrant namespace/collection payload family, and retrieval policy path; memory must not become a source adapter or pollute source retrieval without explicit memory search/context intent.
