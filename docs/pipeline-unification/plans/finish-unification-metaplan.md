...
| `axon-crawl` split into adapters/route/ledger/document/jobs. | Partial/Open | The old `axon-crawl` workspace crate is absent, and crawl jobs use the unified job store, but `CrawlRunner` still executes the legacy web-engine crawl-to-disk path plus embed handoff. Full `SourceRequest -> WebSourceAdapter -> ledger -> prepare/embed/publish` runner migration remains open. |
...
| Move/wrap/rewrite/delete rules applied with no legacy facades. | Open | Compatibility/public surface removal is incomplete, and crawl still has a legacy engine-backed runner even though the old split crate is gone. |
