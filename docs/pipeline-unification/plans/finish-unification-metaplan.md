...
| Move vertical extraction parse facts into `axon-parse`/`axon-adapters`; keep structured `extract` orchestration in `axon-services`. | Partial | `DocumentPreparer` calls `axon_parse::builtins::production_registry()` when facts are absent, and parse facts flow into `PreparedDocument`. Need verify each vertical source emits/uses specific facts, not just generic fallback. |
