...
| `axon-crawl` split into adapters/route/ledger/document/jobs. | Implemented for web SourceRequest path / migration bridge remains | The old `axon-crawl` workspace crate is absent. Site/page/docs web acquisition now runs through Source jobs and the web adapter/source pipeline. Legacy `JobKind::Crawl` rows are migration-only and are dead-lettered with `legacy.crawl.removed` rather than recovered/requeued. |
