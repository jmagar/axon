...
| Remove old `embed`, `ingest`, `crawl`, `code-search`, `code-search-watch`, `purge`, `dedupe`, and legacy MCP action families from normal public surfaces; restore `axon scrape <url>` only as the canonical one-page SourceRequest projection. | Partial/Open | `embed/ingest/crawl/code-search` are absent from current CLI parsing, but `scrape` also needs to be restored as `scope=page`/`embed=true`, and `purge`/`dedupe` remain in `CommandKind`, clap parsing, and CLI dispatch. Generated docs/clients still reference old REST/MCP routes/actions. |
...
| Ensure REST OpenAPI includes every end-state route and excludes every removed route. | Open | Generated docs/clients still reference `/v1/scrape`, `/v1/crawl`, `/v1/embed`, `/v1/ingest`, `/v1/dedupe`, `/v1/purge`, and `/v1/watch/{id}/run`; `/v1/scrape` remains removed even though CLI `axon scrape <url>` is retained as a SourceRequest projection. |
...
| `axon-crawl` split into adapters/route/ledger/document/jobs. | Partial/Open | The old `axon-crawl` workspace crate is absent, and crawl jobs use the unified job store, but `CrawlRunner` still executes the legacy web-engine crawl-to-disk path plus follow-up embed handoff. Crawls still auto-embed when `embed=true`; the current fail-closed handoff means the crawl fails instead of reporting success if markdown output is missing, config snapshotting fails, or the follow-up embed job cannot be enqueued. Full `SourceRequest -> WebSourceAdapter -> ledger -> prepare/embed/publish` runner migration remains open. |
...
| Move/wrap/rewrite/delete rules applied with no legacy facades. | Open | Compatibility/public surface removal is incomplete, `scrape` needs restoration as a canonical SourceRequest projection, and crawl still has a legacy engine-backed runner even though the old split crate is gone. |
