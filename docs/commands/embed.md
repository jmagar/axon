...
- In server mode (`AXON_SERVER_URL` / `--server-url`), URL and text inputs are submitted to `axon serve`. Host-local paths such as `./README.md` are rejected with a clear error because the server normally runs in a Docker container with a different filesystem. Use `--local` for local file/directory embedding until upload/import support is added.
