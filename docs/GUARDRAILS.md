...
- Docker Compose reads repo-root `.env` automatically for `${VAR}` interpolation; service containers receive credentials via the repo-root `services.env` referenced by `config/docker-compose.services.yaml`
