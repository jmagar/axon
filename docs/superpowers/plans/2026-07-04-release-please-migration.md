...
- Do not delete `cargo xtask bump-version`, `regen-changelog`, `cliff.toml`, or git-cliff code until after the first successful release-please release.
...
- Modify `xtask/src/main.rs`: add `release-please-dispatch-plan`; keep `bump-version` and `regen-changelog` during the trial.
...
## Task 9: Trial Release-Please Without Removing git-cliff Fallback
...
**Interfaces:**
- Produces: operator docs that prefer release-please but retain fallback during trial.

- [ ] **Step 1: Update docs**

In `CLAUDE.md`, change release instructions to:

```markdown
Release PRs are generated by release-please after `CI` succeeds on `main`.
Do not run `cargo xtask bump-version` for normal releases during the release-please trial.
The old git-cliff-backed command remains only as a rollback tool until one release-please release succeeds end-to-end.
```

- [ ] **Step 2: Keep old files**

Run:

```bash
test -f cliff.toml
test -f xtask/src/checks/release_versions/cliff.rs
cargo xtask bump-version --help >/dev/null
```

Expected: all pass.

- [ ] **Step 3: Validate docs links and symlinks**
...
git commit -m "docs: document release-please trial flow"
```

Only add `AGENTS.md` and `GEMINI.md` if symlinks changed.

## Task 10: Post-First-Release Cleanup
...
**Interfaces:**
- Consumes: proof that release-please created a correct release PR, tag, GitHub Release, and artifact uploads.
- Produces: final removal of git-cliff and manual bumping.

- [ ] **Step 1: Verify first release**

Run:

```bash
gh release view v6.2.2 --json tagName,isDraft,isPrerelease,assets
gh run list --workflow=release-please.yml --branch main --limit 5
```

Use the actual first release tag if it is not `v6.2.2`.

Expected: release exists, is not draft, and has expected assets attached.

- [ ] **Step 2: Remove old commands**

Remove from `xtask/src/main.rs`:
...
Remove from `xtask/src/checks/release_versions.rs`:

```rust
mod cliff;
pub enum BumpLevel { Patch, Minor, Major }
pub fn bump(...)
pub fn regen_changelog(...)
```

- [ ] **Step 3: Delete git-cliff files**

Run:

```bash
git rm cliff.toml xtask/src/checks/release_versions/cliff.rs xtask/src/checks/release_versions/cliff_tests.rs
```
...
```bash
git add xtask/src/main.rs xtask/src/checks/release_versions.rs xtask/src/checks/release_versions_tests.rs CLAUDE.md Cargo.lock
...
- [x] 12. Delay git-cliff removal until after a proven release.

## Self-Review

**Spec coverage:** This plan switches normal release PR/version/changelog flow to release-please, keeps artifact workflows, preserves CI gating, and removes git-cliff/xtask bumping only after release-please proves itself.
