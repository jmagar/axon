...
- `subaction` is optional for submit families (`extract`); when omitted, handlers default to `start`
...
For job lifecycle management (`get|events|cancel|retry|cleanup|recover`), use the canonical `jobs` action:

```json
{ "action": "jobs", "subaction": "get", "job_id": "..." }
```
...
| `source` | -- | `source` (aliases: `input`), `scope`, `collection`, `response_mode`, `detached` |
