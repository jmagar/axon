...
| `chat` | `core::llm` direct completion path | no dedicated action | `POST /v1/chat`, `POST /v1/chat/stream` = Implemented | Direct LLM chat without RAG retrieval/synthesis prompt. Used by clients that need plain model chat. |
