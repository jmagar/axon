...
> Current generated runtime snapshot. The clean-break source-pipeline target is
> tracked in [`../../pipeline-unification/`](../../pipeline-unification/README.md)
> and GitHub issue #298. Do not infer future compatibility aliases from this
> page.
...
|---|---|---|
| CLI | `axon <action> ...` and lifecycle subcommands | `crates/axon-cli/src/commands/`, `src/lib.rs` |
| Service | Typed request/result structs shared by adapters | `crates/axon-services/src/` |
| REST | Direct `/v1/<action>` routes and lifecycle job routes | `crates/axon-web/src/`, `docs/reference/api-parity.md` |
| MCP | One `axon` tool with `action` and optional `subaction` | `crates/axon-api/src/mcp_schema.rs`, `docs/reference/mcp/tool-schema.md` |
