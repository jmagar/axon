...
async function fetchAllPoints() {
    ...
    let offset = null;
    const limit = 100;
    console.log(`Fetching points from ${QDRANT_URL}/collections/${COLLECTION}...`);
    while (true) {
        const scrollBody = {
            limit,
            with_payload: true,
 ...
 */
function findDuplicates(points) {
...
async function main() {
    ...
    if (duplicates.length === 0) {
        ...
        const totalDuplicatePoints = duplicates.reduce((sum, d) => sum + (d.count - 1), 0);
    ...
    console.log(`Unique URLs: ${new Set(points.map(p => p.payload.url).filter(Boolean)).size}`);
    console.log(`Duplicate chunks: ${duplicates.length}`);
    // Calculate average chunks per URL
    const urlChunkCounts = new Map();
    for (const point of points) {
        const url = point.payload.url;
        if (!url)
            continue;
