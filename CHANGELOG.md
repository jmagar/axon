...
- **`ask` context budget now scales with the configured LLM's context window** (`tuning.rs`). The `ask_max_context_chars` default is derived from the model instead of a fixed 300k: Gemini/Claude (≈1M-token windows) → 1,000,000 chars; Codex (≈400k) → 400,000; unknown models → 40,000 (assume a <50k-token window). Detection: the `gemini-headless` backend is always Google; otherwise the `AXON_OPENAI_MODEL` name is matched (`gemini`/`gemma`/`claude`/`codex`). An explicit `AXON_ASK_MAX_CONTEXT_CHARS` env or `ask.max-context-chars` TOML value still overrides. A practical effect: large authoritative full documents now fit on big-context models (e.g. the 21KB canonical plugins page enters as a complete full-doc on Gemini instead of a single supplemental chunk).
