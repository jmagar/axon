...
- **Conditional re-crawl (ETag / If-Modified-Since)** — opt-in via `--etag-conditional` (bead axon_rust-hiyf). Re-crawls seed spider 2.51's per-`Website` ETag cache from a persisted `etag.json` sidecar so unchanged pages return `304 Not Modified`. Because spider drops 304 responses from its broadcast stream entirely, the crawl engine **reconciles** those silent skips back into the manifest: every URL that was seeded with validators and did not arrive this run is re-emitted as a reused (`changed=false`) entry, relinked from the `markdown.old` recycling bin. The reconciliation set is scoped to `{seeded ∩ previous_manifest − arrived}`, which is empty without a working seed — so the feature can never resurrect genuinely-removed pages into zombie content. Validators are carried forward across runs (spider's 304 path returns before re-storing, so the previous sidecar is overlaid with freshly-stored validators). Requires `--cache` (default on) for the archive. Wired on the crawl path only; single-page `scrape` is intentionally excluded (a 304 there would drop the page the user asked for, with no reconciliation seam).
