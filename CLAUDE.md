...
- `docs/pipeline-unification/` and
  https://github.com/jmagar/axon/issues/298 are the future clean-break
  source-pipeline contracts. The command table below is current runtime
  behavior, not a compatibility promise for the pipeline-unification cutover.
...
| `embed [input]` | Embed file/dir/URL into Qdrant; existing local paths start a background local code-index watcher by default (`--watch` attaches foreground progress, `--no-watch` keeps one-shot local embedding) | Yes (default) |
...
**Watch scheduler:** `watch list`, `watch create`, `watch run-now`, and `watch history` are wired through `crates/axon-services/src/watch.rs` → `crates/axon-jobs/src/watch.rs` and work today. A `watch` task (the only supported `task_type`) is a **URL change detector**: each run probes/scrapes every URL, filters noise (`ignore_patterns`), reuses `services::diff::compute_diff` against the stored `axon_watch_url_state` snapshot, applies a meaningfulness threshold, summarizes meaningful changes via the Gemini `core/llm` backend, writes `url-change` artifacts, and enqueues one crawl per common-prefix cluster (in-flight-guarded). The change-detection logic lives in `crates/axon-jobs/src/watch/{change_detect,cluster,dispatch,filter,report,url_state,orchestrate}.rs` with the conditional probe in `crates/axon-core/src/http/conditional.rs`. Enabled watches also **fire automatically**: the in-process scheduler loop in `crates/axon-jobs/src/workers/watch_scheduler.rs` (spawned by `spawn_workers`, so active under `axon serve`/`axon mcp`) leases due watches (`next_run_at <= now`) each `AXON_WATCH_TICK_SECS` and runs them, advancing `next_run_at` by `every_seconds`. `watch get`, `watch update`, `watch pause`, `watch resume`, `watch delete`, and `watch artifacts` parse but are not yet implemented. The #298 target replaces this URL-only watch model with source-request-backed watches.
...
Releases are **per-component and selective**. Release PRs are generated by
release-please after `CI` succeeds on `main`. In manifest mode,
release-please owns release PRs, version edits, changelog edits, tags, and
GitHub Release records. Axon's artifact workflows attach binaries/APKs/zips to
the release-please-created release by tag.

`release/components.toml` remains Axon's local validation and artifact dispatch
source of truth during the release-please trial. It defines component shipping
paths, tag prefixes, release workflows, version sources, version-bearing files,
and release-please package paths:
...
| **chrome** (extension zip) | `apps/chrome-extension` | `apps/chrome-extension/manifest.json` `version` | `chrome-ext-v` | `chrome-extension-release.yml` |

For each component, the shared release checker diffs that component's shipping
paths against its most recent tag and verifies the release-please manifest,
component version source, and all version-bearing files agree. After
release-please creates a release, `.github/workflows/release-please.yml`
dispatches the component artifact workflow for each released path. Those
workflows build, package with SHA256 checksums, and upload assets to the
existing release.
...
2. Review that the release PR updates `.release-please-manifest.json`, component versions, and changelogs.
