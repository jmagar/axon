...
use super::feed_source_adapter::{FeedAdapterRun, source_summary, timestamp};
use super::feed_source_progress::{FeedSourceProgress, progress_error_context};
use super::{FEED_LEASE_TTL_SECONDS, FeedSourceIndexInput};
...
pub(super) async fn ensure_lease_before_publish(
    ...
    if let Err(fail_err) = ledger.fail_generation(generation).await {
        return Err(anyhow::anyhow!(
            "feed source refresh lost lease before publish and failed to mark generation failed: {fail_err}"
        ));
    }
    Err(anyhow::anyhow!(
        "feed source refresh lost lease before publish"
    ))
}
