...
use super::FeedSourceIndexInput;
use super::feed_source_adapter::{FeedAdapterRun, normalize_changed_documents, timestamp};
use super::feed_source_progress::{
    FeedSourceProgress, progress_error_context, record_progress_with_reservations,
};

const FEED_CHANGED_DOCUMENT_BATCH_SIZE: usize = 64;
const FEED_CHANGED_CHUNK_BATCH_SIZE: usize = 512;
...
pub(super) async fn vectorize_changed_documents(
    ...
    for batch_diff in changed_diff_batches(diff, FEED_CHANGED_DOCUMENT_BATCH_SIZE) {
        let documents = prepare_changed_documents(run, &batch_diff, generation).await?;
        for prepared_batch in prepared_document_batches(documents, FEED_CHANGED_CHUNK_BATCH_SIZE) {
...
async fn vectorize_documents(
    ...
    // "`embed=false` never writes vectors"): entries are still discovered,
