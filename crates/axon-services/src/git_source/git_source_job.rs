...
use super::git_source_adapter::{git_source_id, timestamp};
use super::git_source_progress::{GitSourceProgress, source_error_from_api_error};
use super::{GitSourceIndexInput, GitSourceIndexOutput, index_git_source_with_progress};

pub async fn index_git_source_with_job(
    mut input: GitSourceIndexInput,
    jobs: &dyn JobStore,
    ledger: &dyn LedgerStore,
    embedding_provider: &dyn EmbeddingProvider,
    vector_store: &dyn VectorStore,
) -> anyhow::Result<GitSourceIndexOutput> {
    let target = axon_adapters::git::parse_git_target(&input.target_url)
    ...
    match index_git_source_with_progress(
        input.clone(),
        ledger,
        embedding_provider,
        vector_store,
    ...
    .await
    {
        ...
        Err(err) => {
            let source_error = terminal_source_error(&err, &input.repo_root);
                ...
                .await
            {
                return Err(err.context(format!(
                    "also failed to record terminal git source job failure: {progress_err}"
                )));
            }
            Err(err)
        }
...
fn terminal_source_error(err: &anyhow::Error, repo_root: &Path) -> SourceError {
    if let Some(api_error) = err.downcast_ref::<ApiError>() {
        return source_error_from_api_error(api_error);
    }
    let safe_error = redact_repo_root(&err.to_string(), repo_root);
    SourceError {
        code: "source.git.index_failed".to_string(),
...
fn redact_repo_root(message: &str, repo_root: &Path) -> String {
    let mut redacted = message.to_string();
    let root_display = repo_root.display().to_string();
    if !root_display.is_empty() {
        redacted = redacted.replace(&root_display, "<git-source-repo-root>");
    }
    if let Ok(canonical) = std::fs::canonicalize(repo_root) {
        ...
        if !canonical_display.is_empty() {
            redacted = redacted.replace(&canonical_display, "<git-source-repo-root>");
        }
    }
    redacted
}
...
#[async_trait]
impl GitSourceProgress for JobProgressSink<'_> {
        ...
        *sequence += 1;
        ...
        self.jobs
            .update_status(JobStatusUpdate {
                ...
                counts: counts.clone(),
                current: None,
                message: Some(format!("git source {phase:?}").to_ascii_lowercase()),
                error: error.clone(),
            })
            .await?;
        ...
        self.jobs
            .append_event(SourceProgressEvent {
                event_id: format!("evt_git_{}_{}", self.job_id.0, sequence),
                sequence,
                job_id: self.job_id,
                attempt: 1,
                stage_id: None,
                ...
                message: format!("git source {phase:?}").to_ascii_lowercase(),
                timestamp: timestamp(),
                source_id: Some(self.source_id.clone()),
                canonical_uri: None,
                adapter: None,
        ...
        self.jobs
            .heartbeat(JobHeartbeat {
                job_id: self.job_id,
                attempt: 1,
                worker_id: Some("git-source".to_string()),
                phase,
                status,
                stage_id: None,
...
fn counts_for_output(output: &GitSourceIndexOutput) -> StageCounts {
    StageCounts {
        items_total: None,
        items_done: 0,
