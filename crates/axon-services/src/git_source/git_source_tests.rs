...
/// A repo with a single `.txt` file. `ContentKind::PlainText` (what git's
/// `content_kind_for` assigns unknown extensions) is matched by **no** built-in
...
/// candidate cleanly.
fn fixture_repo_unparsed_text() -> PathBuf {
    let dir = std::env::temp_dir().join(format!("axon-git-source-enrich-test-{}", Uuid::new_v4()));
    fs::create_dir_all(&dir).unwrap();
    fs::write(
        dir.join("page.txt"),
        "hello world with nothing for any parser to latch onto\n",
    )
    .unwrap();
    dir
}
...
/// the stage was called. Uses `embed=false` to keep this focused on the
/// acquire/enrich/normalize/prepare handoff rather than vector-payload schema
/// validation, and uses `FakeSourceEnricher` (not `NoopSourceEnricher`) so the
/// assertion exercises real enrichment output flowing downstream.
#[tokio::test]
async fn git_source_enrichment_graph_candidates_flow_into_index_output() {
    let repo = fixture_repo_unparsed_text();
    let ledger = FakeLedgerStore::new();
    let embedder = FakeEmbeddingProvider::new("fake-embedding", 8);
    let vectors = FakeVectorStore::new("fake-vector");
    ...
    let mut input = input_with_enricher(repo.clone(), enricher.clone());
    input.embed = false;

    let output = index_git_source(input, &ledger, &embedder, &vectors)
        .await
        .unwrap();
