...
use super::registry_source_adapter::{registry_source_id, timestamp};
use super::registry_source_progress::{RegistrySourceProgress, source_error_from_api_error};
use super::{
    RegistrySourceIndexInput, RegistrySourceIndexOutput, index_registry_source_with_progress,
};

pub async fn index_registry_source_with_job(
    mut input: RegistrySourceIndexInput,
    jobs: &dyn JobStore,
    ...
    match index_registry_source_with_progress(
        input.clone(),
        ledger,
        embedding_provider,
        vector_store,
    ...
    .await
    {
        ...
        Err(err) => {
            let source_error = terminal_source_error(&err, &input.registry_dump_path);
                ...
                .await
            {
                return Err(err.context(format!(
                    "also failed to record terminal registry source job failure: {progress_err}"
                )));
            }
            Err(err)
        }
...
fn terminal_source_error(err: &anyhow::Error, dump_path: &Path) -> SourceError {
    if let Some(api_error) = err.downcast_ref::<ApiError>() {
        return source_error_from_api_error(api_error);
    }
    let safe_error = redact_dump_path(&err.to_string(), dump_path);
    SourceError {
        code: "source.registry.index_failed".to_string(),
...
fn job_create_request(input: &RegistrySourceIndexInput, _source_id: SourceId) -> JobCreateRequest {
    JobCreateRequest {
        request_id: None,
...
fn redact_dump_path(message: &str, dump_path: &Path) -> String {
    let mut redacted = message.to_string();
    let path_display = dump_path.display().to_string();
    if !path_display.is_empty() {
        redacted = redacted.replace(&path_display, "<registry-dump-path>");
    }
    if let Ok(canonical) = std::fs::canonicalize(dump_path) {
        let canonical_display = canonical.display().to_string();
        if !canonical_display.is_empty() {
            redacted = redacted.replace(&canonical_display, "<registry-dump-path>");
        }
    }
    redacted
}
...
#[async_trait]
impl RegistrySourceProgress for JobProgressSink<'_> {
        ...
        *sequence += 1;
        ...
        self.jobs
            .update_status(JobStatusUpdate {
                ...
                counts: counts.clone(),
                current: None,
                message: Some(format!("registry source {phase:?}").to_ascii_lowercase()),
                error: error.clone(),
            })
            .await?;
        ...
        self.jobs
            .append_event(SourceProgressEvent {
                event_id: format!("evt_registry_{}_{}", self.job_id.0, sequence),
                sequence,
                job_id: self.job_id,
                attempt: 1,
                stage_id: None,
                ...
                message: format!("registry source {phase:?}").to_ascii_lowercase(),
                timestamp: timestamp(),
                source_id: Some(self.source_id.clone()),
                canonical_uri: None,
                adapter: None,
        ...
        self.jobs
            .heartbeat(JobHeartbeat {
                job_id: self.job_id,
                attempt: 1,
                worker_id: Some("registry-source".to_string()),
                phase,
                status,
                stage_id: None,
...
fn counts_for_output(output: &RegistrySourceIndexOutput) -> StageCounts {
    StageCounts {
        items_total: None,
        items_done: 0,
