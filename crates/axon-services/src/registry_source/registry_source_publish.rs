...
use super::registry_source_adapter::{RegistryAdapterRun, source_summary, timestamp};
use super::registry_source_progress::{RegistrySourceProgress, progress_error_context};
use super::{REGISTRY_LEASE_TTL_SECONDS, RegistrySourceIndexInput};
...
pub(super) async fn ensure_lease_before_publish(
    ...
    if let Err(fail_err) = ledger.fail_generation(generation).await {
        return Err(anyhow::anyhow!(
            "registry source refresh lost lease before publish and failed to mark generation failed: {fail_err}"
        ));
    }
    Err(anyhow::anyhow!(
        "registry source refresh lost lease before publish"
    ))
}
