mod youtube_source_adapter;
mod youtube_source_job;
mod youtube_source_progress;
mod youtube_source_publish;
mod youtube_source_vectorize;
...
pub(super) use self::youtube_source_adapter::youtube_source_id;
use self::youtube_source_adapter::{
    YoutubeAdapterRun, collection_spec, discover_manifest, resolve_adapter_run, source_summary,
    timestamp,
};
#[allow(unused_imports)]
pub use self::youtube_source_job::index_youtube_source_with_job;
use self::youtube_source_progress::{
    YoutubeSourceProgress, ensure_providers_ready, phase_for_api_error, record_progress,
    record_progress_error,
};
use self::youtube_source_publish::{
...
use self::youtube_source_vectorize::{
    VectorizeResult, publish_document_status, vectorize_changed_documents,
};

const YOUTUBE_ADAPTER_VERSION: &str = env!("CARGO_PKG_VERSION");
const YOUTUBE_LEASE_TTL_SECONDS: u64 = 30 * 60;

/// Inputs for indexing one YouTube source generation. `youtube_dump_path`
/// points at a prepared JSON dump of video metadata + transcripts (produced
/// by the caller's `yt-dlp` fetch), mirroring how `LocalSourceIndexInput`
/// takes a filesystem `root` and the git adapter takes a `repo_root`.
#[derive(Debug, Clone)]
pub struct YoutubeSourceIndexInput {
    pub target: String,
    pub youtube_dump_path: PathBuf,
...
#[cfg(test)]
pub async fn index_youtube_source(
    input: YoutubeSourceIndexInput,
    ledger: &dyn LedgerStore,
    embedding_provider: &dyn EmbeddingProvider,
    vector_store: &dyn VectorStore,
) -> anyhow::Result<YoutubeSourceIndexOutput> {
    index_youtube_source_with_progress(input, ledger, embedding_provider, vector_store, None).await
}

async fn index_youtube_source_with_progress(
    input: YoutubeSourceIndexInput,
    ledger: &dyn LedgerStore,
    embedding_provider: &dyn EmbeddingProvider,
    vector_store: &dyn VectorStore,
    progress: Option<&dyn YoutubeSourceProgress>,
) -> anyhow::Result<YoutubeSourceIndexOutput> {
    ...
    let lease = ledger
        .acquire_lease(LeaseRequest {
            ...
            ttl_seconds: YOUTUBE_LEASE_TTL_SECONDS,
        ...
        .ok_or_else(|| {
            anyhow::anyhow!(
                "youtube source refresh already running for {}",
                run.source_id.0
            )
        })?;

    let result = index_youtube_source_with_lease(
        &input,
        ledger,
        embedding_provider,
        vector_store,
    ...
    match (result, release) {
        ...
        (Err(err), Ok(())) => Err(err),
        (Ok(_), Err(err)) => {
            Err(anyhow::Error::new(err).context("failed to release youtube source lease"))
        }
        (Err(err), Err(release_err)) => Err(err.context(format!(
            "additionally failed to release youtube source lease: {release_err}"
        ))),
    }
}

async fn index_youtube_source_with_lease(
    input: &YoutubeSourceIndexInput,
    ledger: &dyn LedgerStore,
    embedding_provider: &dyn EmbeddingProvider,
    vector_store: &dyn VectorStore,
...
async fn unchanged_refresh_output(
...
) -> anyhow::Result<Option<YoutubeSourceIndexOutput>> {
    ...
    ledger
        ...
        .await?;
    Ok(Some(YoutubeSourceIndexOutput {
        job_id: input.job_id,
...
async fn ensure_lease_before_publish_or_rollback_vectors(
    ...
    if let Err(err) = ensure_lease_before_publish(ledger, input, lease, generation.clone()).await {
        ...
        {
            return Err(err.context(format!(
                "also failed to rollback youtube source generation vectors: {rollback_err}"
            )));
        }
        return Err(err);
    }
...
#[cfg(test)]
#[path = "youtube_source_failure_tests.rs"]
mod failure_tests;
#[cfg(test)]
#[path = "youtube_source_refresh_tests.rs"]
mod refresh_tests;
#[cfg(test)]
#[path = "youtube_source_tests.rs"]
mod tests;
