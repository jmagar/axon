//! Ingest-only bridge from the unified `JobStore` onto the legacy `ServiceJob`
//! shape.
//!
//! `JobKind::Ingest` now enqueues and executes on the unified job store (see
//! `IngestRunner` in `crates/axon-services/src/runtime/job_runners/
//! ingest_runner.rs`), but every CLI/MCP/REST caller still renders through
//! `ServiceJob`. This module mirrors `extract_bridge.rs`/`embed_bridge.rs`/
//! `crawl_bridge.rs`: it converts unified `JobSummary`/result DTOs into
//! `ServiceJob` so those shared renderers keep working unchanged for Ingest.
...
/// Ingest's stored `request` payload shape: `{"source": <IngestSource>,
/// "source_type": "...", "target": "...", "config_json": "..."}` (see
/// `ingest_start_with_context` in `crates/axon-services/src/ingest.rs`).
/// Pulls `source_type`/`target` back out so the bridge can populate
/// `ServiceJob.source_type`/`ServiceJob.target` for CLI/MCP/REST rendering.
fn source_fields_from_request_json(
...
fn job_summary_to_service_job(
    ...
    let (source_type, target) = request_json
        .as_ref()
        .map(source_fields_from_request_json)
        .unwrap_or((None, None));
    ServiceJob {
        id: summary.job_id.0,
...
pub(super) async fn list(
    ...
    let page = store
        .list(JobListRequest {
            status: None,
            kind: Some(UnifiedJobKind::Ingest),
            source_id: None,
            watch_id: None,
...
pub(super) async fn cancel(
    ...
    let result = store
        .cancel(
            JobId::new(id),
            JobCancelRequest {
                reason: Some("cancel requested via ingest job surface".to_string()),
                force_after_ms: None,
                actor: None,
            },
        )
