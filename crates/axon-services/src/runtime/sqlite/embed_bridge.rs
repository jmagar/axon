//! Embed-only bridge from the unified `JobStore` onto the legacy `ServiceJob`
//! shape.
//!
//! `JobKind::Embed` now enqueues and executes on the unified job store (see
//! `EmbedRunner` in `crates/axon-services/src/runtime/job_runners.rs`), but
//! every CLI/MCP/REST caller still renders through `ServiceJob` (shared with
//! Crawl/Ingest, which remain on the legacy per-family backend until their own
//! cutover). This module mirrors `extract_bridge.rs`: it converts unified
//! `JobSummary`/result DTOs into `ServiceJob` so those shared renderers keep
//! working unchanged for Embed.
...
/// Embed's stored `request` payload shape:
/// `{"input": "...", "config_json": "..."}` (see `embed_start_with_context`
/// in `crates/axon-services/src/embed.rs`). Pulls `input` back out so the
/// bridge can populate `ServiceJob.target` for CLI/MCP/REST rendering.
fn target_from_request_json(request_json: &serde_json::Value) -> Option<String> {
    request_json
        .get("input")
        .and_then(|v| v.as_str())
        .map(String::from)
...
fn job_summary_to_service_job(
    ...
    let target = request_json.as_ref().and_then(target_from_request_json);
    ServiceJob {
        id: summary.job_id.0,
...
pub(super) async fn list(
    ...
    let page = store
        .list(JobListRequest {
            status: None,
            kind: Some(UnifiedJobKind::Embed),
            source_id: None,
            watch_id: None,
...
pub(super) async fn cancel(
    ...
    let result = store
        .cancel(
            JobId::new(id),
            JobCancelRequest {
                reason: Some("cancel requested via embed job surface".to_string()),
                force_after_ms: None,
                actor: None,
            },
        )
