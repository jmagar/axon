...
use super::SessionsSourceIndexInput;
use super::sessions_source_adapter::{
    SessionsAdapterRun, normalize_changed_documents, sanitize_prepared_chunk_metadata, timestamp,
};
use super::sessions_source_progress::{
    SessionsSourceProgress, progress_error_context, record_progress_with_reservations,
};

const SESSIONS_CHANGED_DOCUMENT_BATCH_SIZE: usize = 64;
const SESSIONS_CHANGED_CHUNK_BATCH_SIZE: usize = 512;
...
pub(super) async fn vectorize_changed_documents(
    ...
    for batch_diff in changed_diff_batches(diff, SESSIONS_CHANGED_DOCUMENT_BATCH_SIZE) {
        let documents = prepare_changed_documents(run, &batch_diff, generation).await?;
        for prepared_batch in
            prepared_document_batches(documents, SESSIONS_CHANGED_CHUNK_BATCH_SIZE)
        {
...
async fn prepare_changed_documents(
    ...
    for document in source_documents {
        ...
        sanitize_prepared_chunk_metadata(&mut prepared);
...
async fn vectorize_documents(
    ...
    // "`embed=false` never writes vectors"): session transcript files are
    // still discovered, normalized, and prepared above this call, but
    // neither the embedding provider nor the vector store may be invoked.
    // Counts still reflect prepared documents/chunks; only `points_written`
    // stays zero.
    if !input.embed {
        for document in documents {
