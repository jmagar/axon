...
use super::sessions_source_adapter::{SessionsAdapterRun, source_summary, timestamp};
use super::sessions_source_progress::{SessionsSourceProgress, progress_error_context};
use super::{SESSIONS_LEASE_TTL_SECONDS, SessionsSourceIndexInput};
...
pub(super) async fn ensure_lease_before_publish(
    ...
    if let Err(fail_err) = ledger.fail_generation(generation).await {
        return Err(anyhow::anyhow!(
            "sessions source refresh lost lease before publish and failed to mark generation failed: {fail_err}"
        ));
    }
    Err(anyhow::anyhow!(
        "sessions source refresh lost lease before publish"
    ))
}
