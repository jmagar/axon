...
use super::sessions_source_adapter::{sessions_source_id, timestamp};
use super::sessions_source_progress::{SessionsSourceProgress, source_error_from_api_error};
use super::{
    SessionsSourceIndexInput, SessionsSourceIndexOutput, index_sessions_source_with_progress,
};

pub async fn index_sessions_source_with_job(
    mut input: SessionsSourceIndexInput,
    jobs: &dyn JobStore,
    ...
    match index_sessions_source_with_progress(
        input.clone(),
        ledger,
        embedding_provider,
        vector_store,
        ...
        Err(err) => {
                ...
                .await
            {
                return Err(err.context(format!(
                    "also failed to record terminal sessions source job failure: {progress_err}"
                )));
            }
            Err(err)
        }
...
fn terminal_source_error(err: &anyhow::Error) -> SourceError {
    ...
    SourceError {
        code: "source.session.index_failed".to_string(),
...
fn job_create_request(input: &SessionsSourceIndexInput, _source_id: SourceId) -> JobCreateRequest {
    JobCreateRequest {
        request_id: None,
        ...
        config_snapshot_id: Some(crate::config_snapshot_hash::config_snapshot_id(
            &crate::config_snapshot_hash::JobConfigSnapshot {
                source_kind: "session",
                source_ref: &format!("{}:{}", input.provider, input.session_id),
...
#[async_trait]
impl SessionsSourceProgress for JobProgressSink<'_> {
        ...
        *sequence += 1;
        ...
        self.jobs
            .update_status(JobStatusUpdate {
                ...
                counts: counts.clone(),
                current: None,
                message: Some(format!("sessions source {phase:?}").to_ascii_lowercase()),
                error: error.clone(),
            })
            .await?;
        ...
        self.jobs
            .append_event(SourceProgressEvent {
                event_id: format!("evt_sessions_{}_{}", self.job_id.0, sequence),
                sequence,
                job_id: self.job_id,
                attempt: 1,
                stage_id: None,
                ...
                message: format!("sessions source {phase:?}").to_ascii_lowercase(),
                timestamp: timestamp(),
                source_id: Some(self.source_id.clone()),
                canonical_uri: None,
                adapter: None,
        ...
        self.jobs
            .heartbeat(JobHeartbeat {
                job_id: self.job_id,
                attempt: 1,
                worker_id: Some("sessions-source".to_string()),
                phase,
                status,
                stage_id: None,
...
fn counts_for_output(output: &SessionsSourceIndexOutput) -> StageCounts {
    StageCounts {
        items_total: None,
        items_done: 0,
