mod registry_source_adapter;
mod registry_source_job;
mod registry_source_progress;
mod registry_source_publish;
mod registry_source_vectorize;
...
pub use self::registry_source_job::index_registry_source_with_job;
use self::registry_source_progress::{
    RegistrySourceProgress, ensure_providers_ready, phase_for_api_error, record_progress,
    record_progress_error,
};
use self::registry_source_publish::{
...
use self::registry_source_vectorize::{
    VectorizeResult, publish_document_status, vectorize_changed_documents,
};

const REGISTRY_ADAPTER_VERSION: &str = env!("CARGO_PKG_VERSION");
const REGISTRY_LEASE_TTL_SECONDS: u64 = 30 * 60;

#[derive(Debug, Clone)]
pub struct RegistrySourceIndexInput {
    pub registry_dump_path: PathBuf,
    ...
    /// "`embed=false` never writes vectors"). When `false`, package
    /// pages/versions are still discovered/normalized/prepared but no
    /// embedding provider or vector store call is made.
    pub embed: bool,
    /// `SourceRequest.limits.max_items` (source-pipeline.md limits contract).
    /// Natural mapping for registries: caps the number of package
    /// pages/versions considered from the discovered manifest, applied
    /// before diffing so items beyond the limit are neither diffed nor
    /// vectorized. `None` means uncapped.
...
#[cfg(test)]
pub async fn index_registry_source(
    input: RegistrySourceIndexInput,
    ledger: &dyn LedgerStore,
    embedding_provider: &dyn EmbeddingProvider,
    vector_store: &dyn VectorStore,
) -> anyhow::Result<RegistrySourceIndexOutput> {
    index_registry_source_with_progress(input, ledger, embedding_provider, vector_store, None).await
}

async fn index_registry_source_with_progress(
    input: RegistrySourceIndexInput,
    ledger: &dyn LedgerStore,
    embedding_provider: &dyn EmbeddingProvider,
    vector_store: &dyn VectorStore,
    progress: Option<&dyn RegistrySourceProgress>,
) -> anyhow::Result<RegistrySourceIndexOutput> {
    ...
    let lease = ledger
        .acquire_lease(LeaseRequest {
            ...
            ttl_seconds: REGISTRY_LEASE_TTL_SECONDS,
        ...
        .ok_or_else(|| {
            anyhow::anyhow!(
                "registry source refresh already running for {}",
                run.source_id.0
            )
        })?;

    let result = index_registry_source_with_lease(
        &input,
        ledger,
        embedding_provider,
        vector_store,
    ...
    match (result, release) {
        ...
        (Err(err), Ok(())) => Err(err),
        (Ok(_), Err(err)) => {
            Err(anyhow::Error::new(err).context("failed to release registry source lease"))
        }
        (Err(err), Err(release_err)) => Err(err.context(format!(
            "additionally failed to release registry source lease: {release_err}"
        ))),
    }
}

async fn index_registry_source_with_lease(
    input: &RegistrySourceIndexInput,
    ledger: &dyn LedgerStore,
    embedding_provider: &dyn EmbeddingProvider,
    vector_store: &dyn VectorStore,
...
async fn unchanged_refresh_output(
...
) -> anyhow::Result<Option<RegistrySourceIndexOutput>> {
    ...
    ledger
        ...
        .await?;
    Ok(Some(RegistrySourceIndexOutput {
        job_id: input.job_id,
...
async fn ensure_lease_before_publish_or_rollback_vectors(
    ...
    if let Err(err) = ensure_lease_before_publish(ledger, input, lease, generation.clone()).await {
        ...
        {
            return Err(err.context(format!(
                "also failed to rollback registry source generation vectors: {rollback_err}"
            )));
        }
        return Err(err);
    }
...
/// Cap the discovered registry manifest to `max_items` package
/// pages/versions (`SourceRequest.limits.max_items`). Applied before
/// diffing so items beyond the cap are never diffed, prepared, or
/// vectorized. `None` leaves the manifest untouched.
