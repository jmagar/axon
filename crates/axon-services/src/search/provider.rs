...
/// Build the Tavily provider, first checking and surfacing the same per-op
/// message `ensure_tavily_configured` has always produced when
/// `cfg.tavily_api_key` is unset.
fn tavily_provider(cfg: &Config, op: &str) -> Result<TavilySearchProvider, SearchError> {
...
/// the Tavily path retries on failure.
