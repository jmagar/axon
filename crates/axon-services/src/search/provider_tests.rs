...
async fn run_search_dedupes_across_pages_against_searxng() {
    ...
    server
        .mock_async(|when, then| {
            ...
            then.status(200).json_body(searx_json(&[
                ("https://a.test/1", "dup", "dup"), // duplicate, must be skipped
                ("https://a.test/3", "3", "c3"),
            ]));
        })
        .await;
...
/// if a Tavily key is also configured — same fallback order the old
/// hand-rolled client used.
#[tokio::test]
...
/// (op="research") — no network call, no retry attempted.
#[tokio::test]
