...
use super::YoutubeSourceIndexInput;
use super::youtube_source_adapter::{YoutubeAdapterRun, normalize_changed_documents, timestamp};
use super::youtube_source_progress::{
    YoutubeSourceProgress, progress_error_context, record_progress_with_reservations,
};

const YOUTUBE_CHANGED_DOCUMENT_BATCH_SIZE: usize = 64;
const YOUTUBE_CHANGED_CHUNK_BATCH_SIZE: usize = 512;
...
pub(super) async fn vectorize_changed_documents(
    ...
    for batch_diff in changed_diff_batches(diff, YOUTUBE_CHANGED_DOCUMENT_BATCH_SIZE) {
        let documents = prepare_changed_documents(run, &batch_diff, generation).await?;
        for prepared_batch in prepared_document_batches(documents, YOUTUBE_CHANGED_CHUNK_BATCH_SIZE)
        {
