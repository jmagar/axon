...
use super::youtube_source_adapter::{YoutubeAdapterRun, source_summary, timestamp};
use super::youtube_source_progress::{YoutubeSourceProgress, progress_error_context};
use super::{YOUTUBE_LEASE_TTL_SECONDS, YoutubeSourceIndexInput};
...
pub(super) async fn ensure_lease_before_publish(
    ...
    if let Err(fail_err) = ledger.fail_generation(generation).await {
        return Err(anyhow::anyhow!(
            "youtube source refresh lost lease before publish and failed to mark generation failed: {fail_err}"
        ));
    }
    Err(anyhow::anyhow!(
        "youtube source refresh lost lease before publish"
    ))
}
