...
use super::youtube_source_adapter::{timestamp, youtube_source_id};
use super::youtube_source_progress::{YoutubeSourceProgress, source_error_from_api_error};
use super::{
    YoutubeSourceIndexInput, YoutubeSourceIndexOutput, index_youtube_source_with_progress,
};

pub async fn index_youtube_source_with_job(
    mut input: YoutubeSourceIndexInput,
    jobs: &dyn JobStore,
    ledger: &dyn LedgerStore,
    embedding_provider: &dyn EmbeddingProvider,
    vector_store: &dyn VectorStore,
) -> anyhow::Result<YoutubeSourceIndexOutput> {
    let source_id = youtube_source_id(&input.target);
    let descriptor = jobs
    ...
    match index_youtube_source_with_progress(
        input.clone(),
        ledger,
        embedding_provider,
        vector_store,
        ...
        Err(err) => {
                ...
                .await
            {
                return Err(err.context(format!(
                    "also failed to record terminal youtube source job failure: {progress_err}"
                )));
            }
            Err(err)
        }
...
fn terminal_source_error(err: &anyhow::Error) -> SourceError {
    ...
    SourceError {
        code: "source.youtube.index_failed".to_string(),
...
fn job_create_request(input: &YoutubeSourceIndexInput, _source_id: SourceId) -> JobCreateRequest {
    JobCreateRequest {
        request_id: None,
...
#[async_trait]
impl YoutubeSourceProgress for JobProgressSink<'_> {
        ...
        *sequence += 1;
        ...
        self.jobs
            .update_status(JobStatusUpdate {
                ...
                counts: counts.clone(),
                current: None,
                message: Some(format!("youtube source {phase:?}").to_ascii_lowercase()),
                error: error.clone(),
            })
            .await?;
        ...
        self.jobs
            .append_event(SourceProgressEvent {
                event_id: format!("evt_youtube_{}_{}", self.job_id.0, sequence),
                sequence,
                job_id: self.job_id,
                attempt: 1,
                stage_id: None,
                ...
                message: format!("youtube source {phase:?}").to_ascii_lowercase(),
                timestamp: timestamp(),
        ...
        self.jobs
            .heartbeat(JobHeartbeat {
                job_id: self.job_id,
                attempt: 1,
                worker_id: Some("youtube-source".to_string()),
                phase,
                status,
                stage_id: None,
...
fn counts_for_output(output: &YoutubeSourceIndexOutput) -> StageCounts {
    StageCounts {
        items_total: None,
        items_done: 0,
