//! Ask LLM answer acquisition (streaming-with-fallback), ported from legacy
//! `axon_vector::ops::commands::ask::output`.
...
/// inside `ask_llm_streaming_ttft*`/`ask_llm_non_streaming` in
/// `completion.rs`) is tagged `Interactive` instead of the process-wide
/// default `Background`, so bulk LLM work (research/extract/summarize
/// synthesis, none of which opts into this scope) cannot starve an in-flight
/// `ask`.
