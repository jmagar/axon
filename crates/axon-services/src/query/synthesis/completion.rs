//! LLM completion plumbing for `ask`/`evaluate` synthesis, ported from legacy
//! `axon_vector::ops::commands::streaming`.
//!
//! Only the subset actually reachable from the `axon-retrieval`-cutover
//! `ask`/`evaluate` paths is ported: the tagged/side-by-side streaming
//! plumbing (`TaggedToken`, `ask_llm_streaming_tagged`,
//! `baseline_llm_streaming_tagged`) backed an evaluate side-by-side terminal
//! UI that was `#[expect(dead_code)]` scaffolding in the legacy crate (never
//! wired up), so it is intentionally not reproduced here.
...
use super::prompt::synthesis_prompt_for_backend;
...
fn ask_completion_request(
    ...
    let req = CompletionRequest::new(format!("Question: {query}\n\nContext:\n{context}"))
        .system_prompt(synthesis_prompt_for_backend(cfg.llm_backend))
        .stream(stream)
...
pub(crate) async fn run_streaming_completion_ttft(
    ...
    run_streaming_completion_inner(cfg, req, print_tokens, true, Option::<fn(&str)>::None).await
}
...
where
    F: FnMut(&str) + Send,
{
    run_streaming_completion_inner(cfg, req, print_tokens, true, Some(on_delta)).await
}
    ...
    F: FnMut(&str) + Send,
{
    ...
    let stream_result = llm::complete_streaming(req, |delta| {
        process_one_delta(&mut state, delta, print_tokens, capture_ttft).map_err(|err| {
...
/// Run a streaming LLM completion through the configured backend.
async fn run_streaming_completion(
    ...
    let (answer, _) =
        run_streaming_completion_inner(cfg, req, print_tokens, false, Option::<fn(&str)>::None)
            .await?;
    Ok(answer)
}

/// Run a non-streaming LLM completion through the configured backend.
...
pub(crate) async fn judge_llm_streaming(
    ...
    run_streaming_completion(cfg, judge_completion_request(cfg, ctx, true), print_tokens).await
}
