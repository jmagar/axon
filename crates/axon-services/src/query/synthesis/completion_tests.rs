...
/// Test-only runner-injected variant of [`ask_llm_streaming`], mirroring
/// legacy `streaming_test_support::ask_llm_streaming_with_runner` minus the
/// dropped tagged plumbing.
async fn ask_llm_streaming_with_runner<R>(
    runner: &R,
    cfg: &Config,
    query: &str,
    context: &str,
...
{
    let req = ask_completion_request(cfg, query, context, true);
    let mut answer = String::new();
    let mut saw_stream_payload = false;
    let response = axon_llm::complete_streaming_with_runner(runner, req, |delta| {
        process_stream_delta(delta, &mut answer, print_tokens, &mut saw_stream_payload)
...
async fn baseline_llm_non_streaming_with_runner<R>(
    runner: &R,
    cfg: &Config,
    query: &str,
) -> Result<String, Box<dyn Error + Send + Sync>>
where
    R: CompletionRunner + ?Sized,
{
    let req = baseline_completion_request(cfg, query, false);
    Ok(axon_llm::complete_text_with_runner(runner, req).await?.text)
}

async fn judge_llm_non_streaming_with_runner<R>(
    runner: &R,
    cfg: &Config,
    ctx: &JudgeContext<'_>,
) -> Result<String, Box<dyn Error + Send + Sync>>
where
    R: CompletionRunner + ?Sized,
{
    let req = judge_completion_request(cfg, ctx, false);
    Ok(axon_llm::complete_text_with_runner(runner, req).await?.text)
}

#[test]
fn rag_and_judge_prompts_mark_sources_untrusted() {
    assert!(super::super::prompt::synthesis_prompt_for_gemini().contains("rag-synthesize"));
    assert!(super::super::prompt::SKILL_MD.contains("untrusted source data"));
...
async fn ask_llm_streaming_with_runner_builds_completion_request_and_collects_tokens() {
    ...
    assert_eq!(
        ...
        CompletionRequest::new("Question: How?\n\nContext:\nContext block")
            .system_prompt(super::super::prompt::synthesis_prompt_for_gemini())
