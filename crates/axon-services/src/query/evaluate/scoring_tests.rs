use super::{
    ...
    rag_underperformed, score_totals_from_analysis,
};
...
#[test]
fn score_totals_detects_rag_loss_in_real_judge_format() {
    // Mirrors the actual judge-prompt output shape ("## Axis  RAG: X/5 | Baseline: X/5").
    let analysis = "\
...
#[test]
fn score_totals_detects_rag_win_in_real_judge_format() {
    let analysis = "\
