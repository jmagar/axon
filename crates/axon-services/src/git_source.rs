//! Git source family services bridge.
//!
//! Wires [`axon_adapters::git::GitSourceAdapter`] into the shared source
//! pipeline: resolve → lease → discover → diff → generation → vectorize →
//! publish → document statuses → source summary, mirroring
//! [`super::local_source`]. The network clone is performed by the caller and
//! handed in as `repo_root`, so this bridge and the adapter stay
//! unit-testable with fixture repositories.

mod git_source_adapter;
mod git_source_job;
mod git_source_progress;
mod git_source_publish;
mod git_source_vectorize;
...
pub use self::git_source_job::index_git_source_with_job;
use self::git_source_progress::{
    GitSourceProgress, ensure_providers_ready, phase_for_api_error, record_progress,
    record_progress_error,
};
use self::git_source_publish::{
...
use self::git_source_vectorize::{
    VectorizeResult, publish_document_status, vectorize_changed_documents,
};

const GIT_ADAPTER_VERSION: &str = env!("CARGO_PKG_VERSION");
const GIT_LEASE_TTL_SECONDS: u64 = 30 * 60;
...
pub struct GitSourceIndexInput {
    ...
    /// adapter/scope/providers). When set, `git_source_adapter::source_plan`
    ...
    /// `SourcePlan` handed to `GitSourceAdapter` instead of rebuilding an
...
pub struct GitSourceIndexOutput {
    pub job_id: JobId,
...
#[cfg(test)]
pub async fn index_git_source(
    input: GitSourceIndexInput,
    ledger: &dyn LedgerStore,
    embedding_provider: &dyn EmbeddingProvider,
    vector_store: &dyn VectorStore,
) -> anyhow::Result<GitSourceIndexOutput> {
    index_git_source_with_progress(input, ledger, embedding_provider, vector_store, None).await
}

async fn index_git_source_with_progress(
    input: GitSourceIndexInput,
    ledger: &dyn LedgerStore,
    embedding_provider: &dyn EmbeddingProvider,
    vector_store: &dyn VectorStore,
    ...
    let lease = ledger
        .acquire_lease(LeaseRequest {
            ...
            ttl_seconds: GIT_LEASE_TTL_SECONDS,
            job_id: Some(input.job_id),
            metadata: MetadataMap::new(),
        })
        .await?
        .ok_or_else(|| {
            anyhow::anyhow!("git source refresh already running for {}", run.source_id.0)
        })?;

    let result = index_git_source_with_lease(
        &input,
        ledger,
        embedding_provider,
        vector_store,
    ...
    match (result, release) {
        ...
        (Err(err), Ok(())) => Err(err),
        (Ok(_), Err(err)) => {
            Err(anyhow::Error::new(err).context("failed to release git source lease"))
        }
        (Err(err), Err(release_err)) => Err(err.context(format!(
            "additionally failed to release git source lease: {release_err}"
        ))),
    }
}

async fn index_git_source_with_lease(
    input: &GitSourceIndexInput,
    ledger: &dyn LedgerStore,
    embedding_provider: &dyn EmbeddingProvider,
    vector_store: &dyn VectorStore,
...
) -> anyhow::Result<Option<GitSourceIndexOutput>> {
...
async fn ensure_lease_before_publish_or_rollback_vectors(
    ...
    if let Err(err) = ensure_lease_before_publish(ledger, input, lease, generation.clone()).await {
        ...
        {
            return Err(err.context(format!(
                "also failed to rollback git source generation vectors: {rollback_err}"
            )));
        }
        return Err(err);
    }
...
#[cfg(test)]
#[path = "git_source/git_source_tests.rs"]
mod tests;
