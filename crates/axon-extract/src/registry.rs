...
pub async fn dispatch_by_url(
    ...
    if auto_enabled(verticals::github_pr::INFO, ctx) && verticals::github_pr::matches(url) {
        return Some(verticals::github_pr::extract(url, ctx).await);
    }
    if auto_enabled(verticals::github_issue::INFO, ctx) && verticals::github_issue::matches(url) {
    ...
    if auto_enabled(verticals::github_repo::INFO, ctx) && verticals::github_repo::matches(url) {
        return Some(verticals::github_repo::extract(url, ctx).await);
    }
    if auto_enabled(verticals::github_release::INFO, ctx) && verticals::github_release::matches(url)
    {
        return Some(verticals::github_release::extract(url, ctx).await);
    }
    if auto_enabled(verticals::reddit::INFO, ctx) && verticals::reddit::matches(url) {
        return Some(verticals::reddit::extract(url, ctx).await);
    }
    if auto_enabled(verticals::pypi::INFO, ctx) && verticals::pypi::matches(url) {
        return Some(verticals::pypi::extract(url, ctx).await);
    }
    if auto_enabled(verticals::npm::INFO, ctx) && verticals::npm::matches(url) {
        return Some(verticals::npm::extract(url, ctx).await);
    }
    if auto_enabled(verticals::crates_io::INFO, ctx) && verticals::crates_io::matches(url) {
        return Some(verticals::crates_io::extract(url, ctx).await);
    }
    if auto_enabled(verticals::docs_rs::INFO, ctx) && verticals::docs_rs::matches(url) {
        return Some(verticals::docs_rs::extract(url, ctx).await);
    }
    if auto_enabled(verticals::docker_hub::INFO, ctx) && verticals::docker_hub::matches(url) {
        return Some(verticals::docker_hub::extract(url, ctx).await);
    }
    if auto_enabled(verticals::huggingface_model::INFO, ctx)
        && verticals::huggingface_model::matches(url)
    {
        return Some(verticals::huggingface_model::extract(url, ctx).await);
    }
    if auto_enabled(verticals::dev_to::INFO, ctx) && verticals::dev_to::matches(url) {
        return Some(verticals::dev_to::extract(url, ctx).await);
    }
    if auto_enabled(verticals::shopify::INFO, ctx) && verticals::shopify::matches(url) {
        return Some(verticals::shopify::extract(url, ctx).await);
    }
    if auto_enabled(verticals::hackernews::INFO, ctx) && verticals::hackernews::matches(url) {
        return Some(verticals::hackernews::extract(url, ctx).await);
    }
    if auto_enabled(verticals::stackoverflow::INFO, ctx) && verticals::stackoverflow::matches(url) {
        return Some(verticals::stackoverflow::extract(url, ctx).await);
    }
    if auto_enabled(verticals::arxiv::INFO, ctx) && verticals::arxiv::matches(url) {
