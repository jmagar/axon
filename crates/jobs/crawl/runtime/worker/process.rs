...
use super::cancel_poll::poll_cancel_key;
...
async fn run_active_crawl_job(
    ...
    let result = async {
        ...
        // Warn when an uncapped crawl exceeds the configured page threshold.
        // Reads AXON_CRAWL_SIZE_WARN_THRESHOLD from the environment on each job
        // so it can be tuned without restarting the worker. 0 disables the warning.
        let size_warn_threshold: u32 = std::env::var("AXON_CRAWL_SIZE_WARN_THRESHOLD")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(10_000);
        if size_warn_threshold > 0
            && ctx.job_cfg.max_pages == 0
            && final_summary.pages_seen > size_warn_threshold
        {
            log_warn(&format!(
                "crawl exceeded {size_warn_threshold} pages without --max-pages cap \
                 url={} pages={} — consider re-running with --max-pages to limit scope",
                ctx.url, final_summary.pages_seen
            ));
        }
