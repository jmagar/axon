...
/// Pull metrics from the SQLite job tables in lite mode.
///
...
pub(super) async fn collect_postgres_metrics(cfg: &Config) -> PostgresMetrics {
    ...
    collect_lite_metrics(&pool).await
}

async fn collect_lite_metrics(pool: &SqlitePool) -> PostgresMetrics {
    let mut m = PostgresMetrics::default();
