...
pub(crate) async fn qdrant_dual_search(
    ...
    let primary_body = build_named_query_body(&primary, limit, candidates, hybrid_enabled);
    let secondary_body = build_named_query_body(&secondary, limit, candidates, hybrid_enabled);
