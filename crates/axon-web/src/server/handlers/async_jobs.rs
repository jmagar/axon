...
#[utoipa::path(
    ...
    responses(
        ...
        (status = 400, description = "Invalid crawl request", body = crate::server::error::ErrorBody),
...
#[utoipa::path(
    ...
    responses(
        ...
        (status = 400, description = "Invalid embed request", body = crate::server::error::ErrorBody),
        (status = 502, description = "Upstream embedding service unavailable", body = crate::server::error::ErrorBody)
    ),
    tag = "jobs"
)]
...
#[utoipa::path(
    ...
    responses(
        ...
        (status = 400, description = "Invalid ingest request", body = crate::server::error::ErrorBody),
        (status = 502, description = "Upstream ingest service unavailable", body = crate::server::error::ErrorBody)
    ),
    tag = "jobs"
)]
...
#[utoipa::path(
    ...
    request_body = axon_ingest::sessions::IngestSessionsPreparedRequest,
    responses(
        (status = 202, description = "Prepared sessions ingest job accepted", body = AcceptedJob),
        (status = 400, description = "Invalid prepared sessions request", body = crate::server::error::ErrorBody),
        (status = 502, description = "Upstream ingest service unavailable", body = crate::server::error::ErrorBody)
    ),
    tag = "jobs"
)]
pub(crate) async fn start_prepared_sessions_ingest(
    State((state, cfg)): State<(AppState, Arc<Config>)>,
    Json(req): Json<axon_ingest::sessions::IngestSessionsPreparedRequest>,
) -> Result<impl IntoResponse, HttpError> {
