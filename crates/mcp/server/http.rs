...
use crate::crates::web::security::{HostAllowlist, host_validation_middleware};
...
pub async fn run_unified_server(
    ...
    enforce_mcp_http_startup_policy(host)?;
    let host_allowlist = HostAllowlist::new(host, port, &cfg.mcp_allowed_origins);
    let panel = Arc::new(crate::crates::web::PanelRuntimeState::initialize(
        host, port,
    )?);
    let setup_required = panel.setup_required();
    let cfg_arc = Arc::new(cfg.clone());
    let web_router = crate::crates::web::router(Arc::clone(&cfg_arc), panel);
    let app = mcp_http_router(cfg)
        .await?
        .merge(web_router)
        .layer(middleware::from_fn_with_state(
