...
- `commands/source.rs`, `commands/sessions.rs`, and source-specific helpers:
  unified source command flow and local convenience commands.
...
- Calls the unified source pipeline through service request DTOs.
...
- Shared command behavior should be centralized in `common.rs`,
  `common_jobs.rs`, and source/action helpers to prevent drift across
  subcommands.
