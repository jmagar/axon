...
use crate::ops::is_lock_busy;
use crate::{ops, query, store, workers};
        ...
        loop {
            match self.job_status(id, kind).await? {
                Some(row) => match row.status {
                    crate::status::JobStatus::Completed
                    | crate::status::JobStatus::Failed
                    | crate::status::JobStatus::Canceled => {
