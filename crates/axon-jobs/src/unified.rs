...
fn descriptor(summary: &JobSummary) -> JobDescriptor {
    JobDescriptor {
...
async fn ensure_job_pool(pool: &SqlitePool, job_id: JobId) -> Result<()> {
...
async fn ensure_job(tx: &mut sqlx::Transaction<'_, sqlx::Sqlite>, job_id: JobId) -> Result<()> {
    let found = sqlx::query_scalar::<_, i64>("SELECT COUNT(*) FROM jobs WHERE job_id = ?")
        .bind(job_id.0.to_string())
        .fetch_one(&mut **tx)
        .await
...
fn row_to_event(row: sqlx::sqlite::SqliteRow) -> Result<JobEvent> {
    Ok(JobEvent {
