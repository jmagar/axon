...
/// Priority-aware interactive-lane starvation SLO (R1-09), distinct from
/// [`detect_and_recover_starvation`]'s generic per-kind age-threshold
/// detector above.
///
/// [`detect_and_recover_starvation`] catches an entire wedged/dead worker
/// lane (pending work, zero running, of any priority). This watchdog instead
/// enforces a fairness SLO on the unified `jobs` table: `interactive`
/// priority is meant to preempt background/bulk work, so a `queued`
/// `interactive` job that has waited longer than `slo_ms` with **no**
/// `interactive` job currently `running` is a fairness violation even if
/// other (lower-priority) lanes are healthy and draining normally.
/// `slo_ms <= 0` disables the check.
pub(super) async fn detect_interactive_starvation(pool: &SqlitePool, slo_ms: i64) {
    ...
    .await
    {
        Ok(count) => count,
        Err(e) => {
            tracing::warn!(error = %e, "interactive starvation watchdog: running-count query failed");
            return;
        }
    };
    if running > 0 {
    ...
    .await
    {
        Ok(row) => row,
        Err(e) => {
            tracing::warn!(error = %e, "interactive starvation watchdog: oldest-queued query failed");
            return;
        }
    };
