...
) -> Result<RedactedPublicWrite<SourceProgressEvent>, ApiError> {
    let mut context = RedactionContext::job_event();
    context.visibility_ceiling = event.visibility;
    let value = serde_json::to_value(event)
        .map_err(|error| serialization_error("serialize", error.to_string()))?;
    let write = redact_public_write(value, &context, &DefaultRedactor::new())?;
    let payload = serde_json::from_value(write.payload)
        .map_err(|error| serialization_error("deserialize", error.to_string()))?;
...
pub fn stamp_event_json(
    ...
    let object = value
        .as_object_mut()
        .ok_or_else(|| serialization_error("serialize", "event is not an object"))?;
    let report = serde_json::to_value(redaction)
        .map_err(|error| serialization_error("serialize", error.to_string()))?;
    let report = report
        .as_object()
        .ok_or_else(|| serialization_error("serialize", "redaction report is not an object"))?;
...
pub fn parse_stamped_event(raw: &str) -> Result<SourceProgressEvent, ApiError> {
    let mut value: serde_json::Value = serde_json::from_str(raw)
        .map_err(|error| serialization_error("deserialize", error.to_string()))?;
    let object = value
        .as_object_mut()
        .ok_or_else(|| serialization_error("deserialize", "event is not an object"))?;
