#[path = "serve_supervisor/model.rs"]
mod model;
#[path = "serve_supervisor/preflight.rs"]
mod preflight;
#[path = "serve_supervisor/runtime.rs"]
mod runtime;
...
pub(super) fn is_internal_bridge_runtime() -> bool {
    runtime::is_internal_bridge_runtime()
}

pub(super) async fn run_supervisor(cfg: &Config) -> Result<(), Box<dyn Error>> {
    runtime::run_supervisor(cfg).await
}

#[cfg(test)]
#[path = "serve_supervisor/tests.rs"]
mod tests;
