...
use crate::schema::{AxonToolResponse, BrandRequest, DiffRequest};
...
use axon_services::{brand as brand_svc, diff as diff_svc};
use rmcp::ErrorData;

impl AxonMcpServer {
    pub(in crate::server) async fn handle_diff(
        &self,
        req: DiffRequest,
    ...
    pub(in crate::server) async fn handle_brand(
        &self,
        req: BrandRequest,
    ) -> Result<AxonToolResponse, ErrorData> {
