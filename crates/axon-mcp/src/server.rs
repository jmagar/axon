...
impl ServerHandler for AxonMcpServer {
    async fn call_tool(
        ...
        // resolved prune/memory authz available to `handle_prune`/
        // `handle_memory` via task-local.
        let tcc = rmcp::handler::server::tool::ToolCallContext::new(self, request, context);
        common::CURRENT_PRUNE_AUTHZ
            .scope(
                prune_authz,
                common::CURRENT_MEMORY_AUTHZ.scope(memory_authz, Self::tool_router().call(tcc)),
            )
            .await
    }
