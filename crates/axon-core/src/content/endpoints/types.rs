//! Web-discovery endpoint types.
//!
//! These describe API endpoints discovered while scraping a page (relative
//! paths, absolute URLs, GraphQL/WebSocket endpoints) and the results of
//! probing them for JSON-RPC 2.0 / OpenRPC / MCP support.
//!
//! They are core-level concepts: the extractor in `core::content::endpoints`
//! produces them, and the services layer re-exports them through
//! `services::types::endpoints` for its own consumers. Wire-format and derives
//! are preserved exactly so serialization is unchanged across the move.
