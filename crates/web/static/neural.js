// Bioluminescent Neural Network Animation
// Inspired by fluorescence microscopy — electric blue palette with volumetric glow
const canvas = document.getElementById('neural-canvas');
const ctx = canvas.getContext('2d');

const COLORS = {
    core:   { r: 210, g: 235, b: 255 },  // white-hot center
    bright: { r: 50,  g: 160, b: 255 },  // electric blue
    mid:    { r: 15,  g: 90,  b: 210 },  // medium blue
    dim:    { r: 8,   g: 45,  b: 140 },  // deep blue
    faint:  { r: 4,   g: 20,  b: 70  },  // barely visible
    // Legacy aliases for UI code compatibility
    primary: { r: 50, g: 160, b: 255 },
    accent:  { r: 100, g: 190, b: 255 },
    subtle:  { r: 30, g: 100, b: 200 },
};
...
class SimplexDrift {
    ...
    get(time) {
        ...
        const x = Math.sin(t + this.offsetX) * 0.3
                + Math.sin(t * 2.3 + this.offsetX * 1.7) * 0.15
                + Math.sin(t * 4.1 + this.offsetX * 0.3) * 0.05;
        const y = Math.sin(t * 0.9 + this.offsetY) * 0.3
...
// Dendrite with curved, organic branching (Bezier curves)
class Dendrite {
    constructor(x, y, angle, length, depth = 0) {
        ...
        // Dendritic spines (small bumps along the dendrite — where synapses form)
        this.spines = [];
        if (depth < 3) {
    ...
    draw(opacity, time, depthScale = 1) {
        ...
        this.branches.forEach(branch => branch.draw(opacity, time, depthScale));
    }
    ...
    updatePosition(newStartX, newStartY) {
...
// Axon with myelin sheaths (gaps = Nodes of Ranvier) and curved path
class Axon {
    constructor(x, y) {
        this.startX = x;
        this.startY = y;
        ...
        // Axon terminal boutons (branching endings)
        this.boutons = [];
    ...
    draw(opacity, depthScale = 1) {
        // Glow pass for entire axon
        ctx.save();
        ctx.globalCompositeOperation = 'lighter';
        ...
        this.segments.forEach((segment, i) => {
            if (segment.isMyelin) {
                // Myelin sheath — translucent tube
                ctx.beginPath();
                ctx.moveTo(segment.startX, segment.startY);
                ctx.lineTo(segment.endX, segment.endY);
                ...
                // Node of Ranvier — bright ring with glow
                ctx.save();
                ctx.globalCompositeOperation = 'lighter';
                const nrg = ctx.createRadialGradient(segment.endX, segment.endY, 0, segment.endX, segment.endY, 8 * depthScale);
        ...
        // Axon terminal boutons with glow
    ...
    updatePosition(newStartX, newStartY) {
        ...
        this.terminalX += dx;
        this.terminalY += dy;
        this.boutons.forEach(b => { b.x += dx; b.y += dy; });
    }
...
// Neuron with Hodgkin-Huxley inspired membrane potential
class Neuron {
    constructor() {
        ...
        // Depth: 0=background (small/dim), 1=foreground (large/bright)
        this.depth = Math.random();

        // Membrane potential model (simplified Hodgkin-Huxley)
        this.potential = -70;
        this.threshold = -55;
        ...
        // Spontaneous activity (some neurons fire randomly)
        this.spontaneousRate = Math.random() < 0.15 ? 0.001 + Math.random() * 0.002 : 0;

        // Excitatory post-synaptic potential accumulator
        this.epsp = 0;
        ...
        // Outgoing connections (filled after all neurons created)
    ...
    update(time, dt) {
        ...
        this.dendrites.forEach(d => d.updatePosition(this.x, this.y));
    ...
    draw(time) {
        const potentialNorm = Math.max(0, Math.min(1,
        ...
        // Depth-based scaling: background neurons smaller/dimmer
        const ds = 0.4 + this.depth * 0.6;   // depthScale
        const da = 0.25 + this.depth * 0.75;  // depthAlpha

        // Draw axon and dendrites
        this.axon.draw(baseOpacity * da, ds);
        this.dendrites.forEach(d => d.draw(baseOpacity * da, time, ds));
        ...
        // Firing flash — massive bloom burst
        if (this.isFiring) {
        ...
        // Soma body — translucent membrane with internal structure
        const offX = -r * 0.25;
        const offY = -r * 0.25;
        ...
        // Internal texture — faint web-like structure
        if (this.depth > 0.4) {
            ctx.save();
            ...
            for (let k = 0; k < 5; k++) {
                ...
                ctx.quadraticCurveTo(
                    ...
                    x + Math.cos(angle2) * r * 0.7, y + Math.sin(angle2) * r * 0.7
                );
        ...
        // Nucleus — bright inner glow with offset
        const nucR = r * 0.4;
        const nucG = ctx.createRadialGradient(
            x + offX * 0.3, y + offY * 0.3, 0,
            x, y, nucR
        );
    ...
    getRandomDendriteTip() {
        const d = this.dendrites[Math.floor(Math.random() * this.dendrites.length)];
        return d.getTip();
    }
}

// Synaptic fiber — thin thread between axon terminal and dendrite tip
class SynapticConnection {
    constructor(preNeuron, postNeuron, preTerminal, dendriteTip) {
...
// Batched connection renderer — glow + crisp fiber layers
function drawConnections(conns, time) {
    const buckets = [[], [], []];
...
// Action potential — small dot with glow halo
class ActionPotential {
    constructor(neuron, connection) {
        this.neuron = neuron;
...
class BackgroundParticle {
    ...
    draw(time) {
        ...
        // Glow halo for brighter/larger particles
...
let lastTime = 0;
let neuralIntensity = 0;
let targetIntensity = 0;

function setNeuralIntensity(target) {
    targetIntensity = Math.max(0, Math.min(1, target));
}

// Sort neurons by depth for painter's algorithm (back to front)
...
function animate(currentTime) {
    ...
    const lerpRate = targetIntensity > neuralIntensity ? 0.12 : 0.02;
    neuralIntensity += (targetIntensity - neuralIntensity) * lerpRate;

    ctx.clearRect(0, 0, canvas.width, canvas.height);

    // Faint ambient glow at center of screen
    const cx = canvas.width / 2, cy = canvas.height / 2;
    const ambR = Math.max(canvas.width, canvas.height) * 0.5;
    const ambG = ctx.createRadialGradient(cx, cy, 0, cx, cy, ambR);
    ambG.addColorStop(0, rgba(COLORS.dim, 0.03 + neuralIntensity * 0.02));
    ambG.addColorStop(1, 'rgba(0,0,0,0)');
    ctx.fillStyle = ambG;
    ctx.fillRect(0, 0, canvas.width, canvas.height);

    canvas.style.opacity = 0.9 + neuralIntensity * 0.1;

    // Background particles (behind neurons)
    particles.forEach(p => {
        p.update(currentTime);
        if (p.z < 0.5) p.draw(currentTime);
    });

    // Connection fibers
    drawConnections(connections, currentTime);

    // Update and draw neurons (sorted back-to-front)
    neurons.forEach(neuron => {
        neuron.update(currentTime, dt);
        neuron.draw(currentTime);

        if (neuron.isFiring && neuron.firePhase === 1 && neuron.fireTimer < dt * 2) {
            const outgoing = neuron.outgoingConnections;
            const maxSignals = 2 + Math.floor(neuralIntensity * 5);
            let sent = 0;
            for (let c = 0; c < outgoing.length && sent < maxSignals; c++) {
                if (Math.random() < outgoing[c].strength * 0.4) {
                    signals.push(new ActionPotential(neuron, outgoing[c]));
                    sent++;
                }
            }
        }

        if (neuralIntensity > 0.1 && !neuron.isFiring && neuron.refractoryTime <= 0) {
            if (Math.random() < neuralIntensity * 0.025) {
                neuron.epsp += 25;
            }
        }
    });
    ...
    while (signals.length > 60) signals.shift();

    // Foreground particles (in front of neurons — creates depth)
    particles.forEach(p => {
        if (p.z >= 0.5) p.draw(currentTime);
    });
...
window.neuralIntensity = neuralIntensity;
window.setNeuralIntensity = setNeuralIntensity;
