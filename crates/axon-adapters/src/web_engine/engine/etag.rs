...
pub async fn persist_next_sidecar(
    ...
    if let Err(e) = write_sidecar(output_dir, &next).await {
        axon_core::logging::log_warn(&format!("etag: failed to persist validator sidecar: {e}"));
    }
}
...
pub async fn reconcile_unmodified(
    ...
    if let Err(e) = manifest.flush().await {
        axon_core::logging::log_warn(&format!("etag: manifest flush after reconcile failed: {e}"));
    }
    if reconciled > 0 {
        axon_core::logging::log_info(&format!(
