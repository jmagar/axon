...
//! payload-family metadata fields (`axon_vectors::payload_families::VECTOR_SOURCE_FAMILY_FIELDS`,
//! `"tool"` entry: `tool_name`, `tool_action`, `tool_side_effect_class`,
//! `tool_output_artifact_id`) onto normalized documents. Mirrors
...
/// `tool_name` packs `"{server}/{tool}"` (matching the parent module's own
/// `graph_nodes_for`'s `stable_key` convention) since the shared `tool`
/// payload family has no dedicated MCP-server field of its own — see
/// `docs/pipeline-unification/sources/metadata-payload.md`'s Source Family
/// Classification table (`tool` is shared by `cli_tool` and `mcp_tool`).
/// `tool_action` is a fixed enum-shaped label (`"metadata"` vs `"call"`,
/// passed in by the caller from `resolved.tool_call_count`), not free text.
pub(super) fn mcp_tool_source_document(
    ...
    metadata.insert("source_type".to_string(), json!("mcp_tool"));
    ...
    // MCP schema/call sources have no documented mutation surface at this
    // layer yet (real invocation is deferred — see the parent module's doc
    // comment), so this is always "none" today.
