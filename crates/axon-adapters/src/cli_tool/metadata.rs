...
//! `git/metadata.rs`/`sessions/metadata.rs`.
...
/// see the crate-level follow-up note on real `Execute`-mode threading in
/// `cli_tool.rs`'s module doc comment.
pub(super) fn cli_tool_source_document(
    ...
    metadata.insert("source_type".to_string(), json!("cli_tool"));
