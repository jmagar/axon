...
// Aliased: `crate::adapter::Result` is a one-argument alias
// (`Result<T> = std::result::Result<T, ApiError>`), whereas this file's own
// helpers return the two-argument `std::result::Result<T, CliToolError>` from
// the parent module in a couple of spots — keeping both names distinct avoids
// any ambiguity.
...
fn normalize_sync(
    ...
    validate_adapter(plan)?;
    let resolved = resolve_metadata(plan)?;
    // "execute" only ever appears once `Execute`-mode threading (see the
    // parent module's doc comment) lands and `resolved.execution_count` can
    // be nonzero.
