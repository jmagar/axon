...
async fn cli_tool_adapter_round_trips_a_tool_source_to_a_document() {
    ...
    // Never executes: no execute scope/allowlist is threaded through
    // `SourcePlan` yet, so this always resolves metadata-only regardless of
    // the requested scope (see `cli_tool.rs`'s module doc comment).
    assert_eq!(
...
async fn cli_tool_adapter_api_scope_still_resolves_metadata_only() {
    // `Api` scope is the contract's execute-capable scope
    // (`may_execute_tools: true` in `family_matrix/scopes_tooling.rs`), but no
    // execute-scope/allowlist threading exists at the `SourceAdapter` layer
    // yet, so it must degrade to the same metadata-only behavior as `Tool`.
    let adapter = CliToolSourceAdapter::new();
