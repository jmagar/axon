...
//! shared source pipeline then reads that prepared path — this
...
fn normalize_feed_target(input: &str) -> String {
    let trimmed = input.trim();
    let rest = ["rss:", "feed:", "atom:"]
        .iter()
        .find_map(|prefix| trimmed.strip_prefix(prefix));
    match rest {
        Some(value) if value.contains("://") => value.trim().to_string(),
        Some(value) => format!("https://{}", value.trim()),
