...
fn apply_extra_does_not_clobber_reserved_keys() {
    use crate::ops::tei::pipeline::apply_extra;
...
fn payload_schema_version_covers_source_document_fields() {
    const _: () = assert!(
        crate::ops::qdrant::PAYLOAD_SCHEMA_VERSION >= 8,
...
fn apply_extra_all_reserved_keys_constant_is_non_empty() {
    use crate::ops::tei::pipeline::RESERVED_PAYLOAD_KEYS;
...
fn payload_schema_version_is_at_least_two() {
    let version = std::hint::black_box(crate::ops::qdrant::PAYLOAD_SCHEMA_VERSION);
    assert!(version >= 2);
}

#[test]
fn pipeline_payload_stamps_schema_version_and_extractor() {
    use crate::ops::qdrant::PAYLOAD_SCHEMA_VERSION;
    let mut payload = serde_json::json!({
...
fn ledger_payload_stamps_authoritative_source_fields() {
    ...
    assert_eq!(payload["source_committed"], false);

    let doc_generation_8 = pipeline_test_doc("https://example.com/doc", vec!["body"], false)
        .with_ledger_payload(
            LedgerPayload::try_new(
                "git:https://example.com/org/repo#main".to_string(),
                "git",
    ...
    let embedded_generation_8 = super::build_embedded_doc_from_vectors(
        doc_generation_8,
        vec![vec![0.1, 0.2]],
        &Config::test_default(),
        VectorMode::Unnamed,
    ...
    .unwrap();
    assert_ne!(
        embedded.points[0]["id"],
        embedded_generation_8.points[0]["id"]
    );
}
