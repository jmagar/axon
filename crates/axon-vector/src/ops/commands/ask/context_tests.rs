...
use crate::ops::ranking::is_low_signal_url;
...
fn retrieved_candidate(url: &str, chunk: &str, score: f64) -> RetrievedCandidate {
    let path = crate::ops::ranking::extract_path_from_url(url);
    RetrievedCandidate {
        candidate: AskCandidate {
            ...
            url_tokens: crate::ops::ranking::tokenize_path_set(&path),
            chunk_tokens: crate::ops::ranking::tokenize_text_set(chunk),
            rerank_score: score,
        },
        chunk_index: Some(7),
...
fn full_doc_selection_prefers_url_entity_matches() {
    ...
    let query_tokens =
        crate::ops::ranking::tokenize_query("setup agents not inherit mcp servers skills");
...
fn full_doc_selection_prefers_exact_procedural_guide_over_adjacent_marketplace_doc() {
    ...
    let query_tokens = crate::ops::ranking::tokenize_query("how do i create a claude code plugin");
...
fn full_doc_selection_normalizes_common_docs_route_shapes() {
    ...
    for canonical_url in cases {
        ...
        let query_tokens =
            crate::ops::ranking::tokenize_query("how do i create a claude code plugin");
...
fn full_doc_selection_prefers_dominant_authoritative_host_over_external_summary() {
    ...
    let query_tokens = crate::ops::ranking::tokenize_query("qdrant hybrid queries");
...
async fn empty_corpus_ask_returns_err_not_panic() {
    ...
    let result = crate::ops::commands::ask::ask_result(&cfg, "what is the meaning of life").await;
