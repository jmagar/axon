...
use super::summarize_citation_validation;
...
pub(super) fn assemble_ask_result(
    cfg: &Config,
    query: &str,
    ctx: &AskContext,
    answer: &str,
...
pub(super) fn build_explain_result(
    cfg: &Config,
    query: &str,
    ctx: &AskContext,
    diagnostics_enabled: bool,
    ...
    AskResult {
        query: query.to_string(),
        answer: String::new(),
        // The explain wire payload omits citation_validation entirely.
        citation_validation: None,
        session: None,
        warnings: ctx.warnings.clone(),
        diagnostics: build_diagnostics(diagnostics_enabled, cfg, ctx),
        explain: ctx.explain.clone(),
        timing_ms: build_timing(
            ctx.retrieval_elapsed_ms,
            ctx.context_elapsed_ms,
            0,
