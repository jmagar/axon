...
`chunk_code()` in `input/code.rs` splits source code at AST boundaries (functions, structs, classes) using tree-sitter grammars. Returns `Option<Vec<String>>` — `None` means **no grammar for the extension**. The `chunk_file` caller (`file_ingest.rs`) falls back to `chunk_text()` when it gets `None` **or** an empty `Some(vec![])` (e.g. an empty supported file), so the fallback covers both cases. Supported: Rust, Python, JavaScript, TypeScript/TSX, Go, Bash, and structured data (JSON, YAML, TOML). Chunk range: 500–2000 chars.
...
**Hash collisions:** With 65,536 buckets, ~12% collision rate for 100 unique terms, ~24% for 200 — roughly half the collision rate of the old 30,522-bucket scheme (authoritative values in `sparse.rs` module doc). Qdrant's IDF weighting mitigates remaining impact — high-IDF terms are unlikely to collide. This is a deliberate trade-off vs. requiring the BERT tokenizer vocabulary.
...
- **VectorMode cache is process-local.** `LazyLock<RwLock<HashMap>>` in `tei/qdrant_store.rs` is populated on first embed/query. `Named` cache hits remain authoritative. Cached legacy `Unnamed` hits are revalidated whenever hybrid search is enabled, so worker processes **self-heal on their next embed/query** — but only when the collection name (`AXON_COLLECTION`) is unchanged before and after migration. When migrating to a new collection name (e.g. `cortex` → `cortex_v2`), update `AXON_COLLECTION` and **restart all worker processes** — the cache is keyed by name, so stale entries for the old name are never revalidated under the new one. (bd axon_rust-d71.2)
...
- **Payload schema versioning.** Existing pre-`axon_rust-lu6a` points are implicit schema version `1`; new upserts carry the current `payload_schema_version = 8` (see `qdrant::PAYLOAD_SCHEMA_VERSION` constant and its doc-comment in `ops/qdrant/utils.rs` for the full version history). Default retrieval applies no version filter — backward-compatible with all existing points. Opt-in callers use `VectorSearchRequest::with_payload_schema_version_min(Some(N))` to scope to newer fields. New keyword `extractor_name` payload field is OPTIONAL — generic crawl/embed paths leave it absent rather than writing a placeholder. Schema v8 normalized planner fields (`chunk_content_kind`, `chunk_locator`, `source_range`, `chunking_fallback`, `code_chunk_source`) are also OPTIONAL in mixed collections; consumers must not assume they exist unless filtering by `payload_schema_version >= 8`. `axon sources --by-schema-version` produces per-version chunk counts via collection scroll.
